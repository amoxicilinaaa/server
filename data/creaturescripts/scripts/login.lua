...
function AutoLootinit(cid)
	if getPlayerStorageValue (cid, 20025) == 1 then
...
function onLogin(cid)
  ...
  if getPlayerCurrentGuardian(cid) then                         
	...
	if getPlayerStorageValue(cid, 45144) - os.time() > 1 then
		sendMsgToPlayer(cid, 27, "VocÃª ainda tem um Experience Booster ativo de "..getPlayerStorageValue(cid, 45145).."%. Ele irÃ¡ acabar em "..convertTime(getPlayerStorageValue(cid, 45144) - os.time())..".")
	end
  ...
  if getPlayerStorageValue(cid, 4126) - os.time() > 0 then
	...
	--    doPlayerSendTextMessage(cid, 27, "Vocï¿½ acabou de ganhar 2 dias de vip pelo evento!!.")
	-- end
	...
	local outfit = {}
	   ...
	   if getPlayerStorageValue(cid, 58769) == -1 then
		...
		setPlayerStorageValue(cid, 1000000, 1) -- charmander
		setPlayerStorageValue(cid, 1000001, 1) -- bulbasaur
		setPlayerStorageValue(cid, 1000002, 1) -- squirtle
    end
    ...
    for s = 1, #storages do
        if not tonumber(getPlayerStorageValue(cid, storages[s])) then
           ...
           doPlayerSendTextMessage(cid, 27, "Desculpe, mas ocorreu um problema no servidor, mas agora estÃ¡ tudo bem.")
        end
    end
    --/////////////////////////////////////////////////////////////////////////--
  ...
  --       end
    ...
    elseif getPlayerStorageValue(cid, 75846) >= 1 then     --alterado v1.9 \/
        ...
        sendMsgToPlayer(cid, 20, "VocÃª foi transferido para sua cidade!")
	else
		doRegainSpeed(cid)  
	end
...
--------------------------------------- Antibot
 ...
 --   end
	...
	-- 		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Gra?as ao vip seu nick agora ser? [VIP] "..name111..", reinicie seu cliente para aplicar o novo nick!")
	-- 		doPlayerSendTextMessage(cid, 27, "Gra?as ao vip seu nick agora ser? [VIP] "..name111..", reinicie seu cliente para aplicar o novo nick!")
	-- 	end	
	-- else
	...
	-- 		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Gra?as ao vip seu nick agora ser? [VIP] "..getCreatureName(cid)..", reinicie seu cliente para aplicar o novo nick!")
	-- 		doPlayerSendTextMessage(cid, 27, "Voc? n?o ? mais vip e sua tag foi retirada, reinicie o cliente para aplicar o nick!")
	-- 	end
	-- end
