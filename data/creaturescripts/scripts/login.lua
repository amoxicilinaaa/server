...
function onLogin(cid)
	...
	if getPlayerStorageValue(cid, 53502) >= 1 then 
		--doPlayerSetLossPercent(cid, PLAYERLOSS_EXPERIENCE, 0)
		doPlayerSendTextMessage(cid, 27, "Voc� possui Bless.")
	else
	    doPlayerSendTextMessage(cid, 27, "Voc� n�o tem Bless.")
	end
  ...
  if getPlayerStorageValue(cid, 4126) - os.time() > 0 then
	...
	if isPremium(cid) then 
	    doPlayerSendTextMessage(cid, 27, "Obrigado por ser vip, voc� tem 10% de bonus no Catch e Exp.")
        doCreatureSetSkullType(cid, 2)
	else
	    doPlayerSendTextMessage(cid, 27, "Seja vip e tenha 10% de b�nus no Catch e Exp.")
	end
	...
	if(accountManager == MANAGER_NONE) then
		...
		if(lastLogin > 0) then
			...
			str = "Sua �ltima visita foi " .. os.date("%a %b %d %X %Y", lastLogin) .. "."
