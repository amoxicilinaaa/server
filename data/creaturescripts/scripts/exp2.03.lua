	...
	if isPlayer(cid) and #getCreatureSummons(cid) <= 0 and type == STATSCHANGE_HEALTHLOSS then
    	...
    	if valor >= getCreatureHealth(cid) then
            ...
            elseif getPlayerStorageValue(cid, 84930) >= 1 then--arena
            	...
            	--doPlayerSendTextMessage(cid, 20, "Voc? morreu na dungeon por Favor volte mais tarde!")
...
-----------------------------------------------------------------------------
local config = {
    sturdy = {                    --Pok?mons que possuem a habilidade Sturdy. Configura??o: ["nome_do_pokemon"] = lookType,
        ["Aggron"] = 1685,
		["Mega Aggron"] = 1798,
    },
    cd = 30,                      --Cooldown da habilidade.
    duration = 8,                 --Dura??o, em segundos, do Sturdy.
    storage = 8402,
}
