local function playerAddExp(cid, exp)
    ...
    local doublexp = 0.5--getGlobalStorageValue(17678) == 1 and 1 or 1 
...
--------------------------------------------------- /\/\
    ...
    if givenexp > 0 then
        for a = 1, #deathList do 
            ...
            if isCreature(pk) then
				...
				--doItemSetAttribute(corpse.uid, "corpseowner", pk)
				-----------fim party------------------
               -- playerAddExp(pk, expTotal)
				--print("exp "..expTotal) 
				--print("exp "..getCreatureName(pk))
            end
        end
    end
	...
	--[[if getPlayerStorageValue(cid, dungeonStr) then
		    print("dungeonStr"..getCreatureName(cid)) 
		    dungeon_type = "Beginner"
			dungeon_id = "Charizard_Beginner" 
			idPos = 1 
		    print(doBossInDungeon(dungeon_type, dungeon_id, idPos))
			--addEvent(doBossInDungeon, 3000, dungeon_type, dungeon_id, idPos)
    end ]]
    return true
end