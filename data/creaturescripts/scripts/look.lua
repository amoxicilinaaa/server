...
function onLook(cid, thing, position, lookDistance)
    ...
    if not isCreature(thing.uid) then
        ...
        if not isPokeball(thing.itemid) then
            -- Verifica se o item está na tabela priceList
            if priceList[iname.name] then
                local qnt = thing.type == 0 and 1 or thing.type
                local calculo = priceList[iname.name].price * qnt
                if thing.type > 1 then
                    str = "Você está vendo "..thing.type.." "..iname.plural.."."
                else
                    str = "Você está vendo "..iname.article.." "..iname.name.."."
                end
                str = str.."\nPreço: $"..calculo.."."
                ...
                return false
            elseif getItemInfo(thing.itemid).price then
