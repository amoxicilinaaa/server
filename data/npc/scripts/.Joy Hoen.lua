...
function creatureSayCallback(cid, type, msg)
    ...
    if msgcontains(msg, "ticket") or msgcontains(msg, "TICKET") then
		...
		if not isPremium(cid) then
			if not (getPlayerItemCount(cid, 2160) >= 25) then
				selfSay("Você precisa de 250k para entrar aqui", cid)
				talkState[talkUser] = 0
				return true
			end
			...
			setPlayerStorageValue(cid, 84926, 1)
			doPlayerAddItem(cid, 25217, 1)
			selfSay("Você acabou de receber seu ticket para viagens, agora poderá ir para Hoenn a hora que quiser, bastará apenas pagar o transporte de 250k caso não seja premium! Deseja {viajar} agora?", cid)
		end
