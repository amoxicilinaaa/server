...
function creatureSayCallback(cid, type, msg)
	...
	if not hasTaskStarted(cid) then
		if (getTaskDificultyByLevel(getPlayerLevel(cid)) < DIFFICULTY_VERY_HARD) then
			selfSay("Olá treinador, você não pode iniciar uma tarefa comigo, você é muito fraco.", cid)
			talkState[talkUser] = 0
		return true
		end
	end
	...
	if (msgcontains(msg, 'iniciar')) then
		...
		elseif getTaskCooldown(cid) > 1 then
			selfSay("Desculpe, você não pode iniciar outra tarefa agora, precisa aguardar "..timeString(getTaskCooldown(cid)).." para que eu possa te entregar novas tarefas.", cid)
			talkState[talkUser] = 0
		else
	...
	elseif (msgcontains(msg, 'entregar')) then
		if hasTaskStarted(cid) then
			if getTaskRemainingTime(cid) < 0 then
				selfSay("Você não conseguiu derrotar os pokémons a tempo e falhou em sua tarefa.", cid)
			...
			if (msgcontains(msg, getTaskNameById(i))) then 
				if hasTaskStarted(cid) then
					...
					break
						...
						selfSay("Você terá que derrotar "..getTaskKillsById(i).." "..getTaskNameById(i).."s em "..getTaskTimeById(i).." minutos ou menos para finalizar, tem certeza?", cid)
	...
	elseif talkState[talkUser] > 100 then
		...
		selfSay("Ok, não perca seu tempo! Você deve derrotar todos esses pokémons e voltar aqui para receber sua recompensa antes que o tempo acabe!", cid)
