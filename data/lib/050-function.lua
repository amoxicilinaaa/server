...
function generateSerial()
 ...
 --        newSerial = newSerialStr .. "-" .. newSerialInt
	-- 	query = db.getResult("select * from pokemon_serial where serial = '" .. newSerial.."'") 
...
function setSerial(item, poke, nick, boost, heldx, heldy, nature) 
 --  --[[  if not boost or boost <= 0 then
	-- 	boost = 0
	-- end
	
	-- if not heldx or heldx <= 0 then
	-- 	heldx = 0
	-- end
	
	-- if not heldy or heldy <= 0 then
	-- 	heldy = 0
	-- end]]
