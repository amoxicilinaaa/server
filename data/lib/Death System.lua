...
local function playerAddExp(cid, exp)
	...
	elseif getGlobalStorageValue(98551801) > 0 then
		doublexp = 4
    end
	-- print(doublexp)
...
local Exps = {
  {minL = 1, maxL = 25, multipler = 40}, 
    ...
    {minL = 126, maxL = 150, multipler = 15}, 
    {minL = 151, maxL = 175, multipler = 10},
    {minL = 176, maxL = 200, multipler = 9},
    {minL = 201, maxL = 225, multipler = 8}, 
    {minL = 226, maxL = 250, multipler = 7},
    {minL = 251, maxL = 275, multipler = 6},
    {minL = 276, maxL = 300, multipler = 5},    
    {minL = 301, maxL = 325, multipler = 4}, 
    {minL = 326, maxL = 350, multipler = 3},
...
local function func(cid, position, corpseid, effect)
    ...
    for slot = 0, (getContainerSize(corpse)-1) do
        ...
        -- if isStone(item.itemid) then
			...
			local arrow_pos = {x=posPlayerLevel.x+1,y=posPlayerLevel.y+1,z=posPlayerLevel.z}
			...
			addEvent(doSendMagicEffect, 1000, arrow_pos, 285)
	...
	for a, loot in pairs(lootList) do
		...
		if math.random(1, 100) <= chanceVipItem then
			if chanceVipItem >= math.random(1, 1000) then
				...
				if item and isContainer(item) then
		    ...
		    if hasTaskStarted(getCreatureMaster(cid)) then
				if (getCreatureName(target):lower() == getStartedTask(getCreatureMaster(cid)):lower()) then
					if (getCurrentTaskKills(getCreatureMaster(cid)) < getTaskKillsById(getStartedTaskId(getCreatureMaster(cid)))) then
						...
						doPlayerSendTextMessage(getCreatureMaster(cid), MESSAGE_STATUS_CONSOLE_BLUE, "[Caça Pokémon]: "..getStartedTask(getCreatureMaster(cid))..", Tarefa completa venha receber sua recompensa..")
					-- return true
					end

					doPlayerSendTextMessage(getCreatureMaster(cid), MESSAGE_STATUS_CONSOLE_BLUE, "[Caça Pokémon]: "..getCurrentTaskKills(getCreatureMaster(cid)).." de "..getTaskKillsById(getStartedTaskId(getCreatureMaster(cid))).." "..getStartedTask(getCreatureMaster(cid)).."s.")
					doPlayerSendTextMessage(getCreatureMaster(cid), MESSAGE_STATUS_CONSOLE_BLUE, "[Tempo de Caça]: Você tem "..timeString(getTaskRemainingTime(getCreatureMaster(cid))).." para concluir esta tarefa.")
				end
			end
			-- // Daily Kill // --
