...
elseif not isPlayer(cid) then
    if table ~= "" then   --alterado v1.6
        ...
        -- min = getSpecialAttack(cid) * (table and table.f or 0) * 0.1   --antes do cl?
...
if spell == "Reflect" or spell == "Mimic" or spell == "Magic Coat" then
    ...
    elseif spell == "Leaves Fury" or tonumber(spell) == 73 then -- trocar o nome pra F?ria de Folhas(ou Leaves Fury) e fazer uma spell diferente pra Leaf Storm
...
--      doMoveInAreaMulti(cid, 98, 967, bulletFly, bulletFlyDano, FLYINGDAMAGE, min, max) -- effect e dano Bug
    ...
    if isMega(cid) then
        ...
        for i = 0, 3 do 
    -- 167 (fucaz?ozinho), 507 (pofff), 929 (cortes), 490 (twist), 800 (corte circ), 894 (areia), 931 azul pof, 933 fura blue
    ...
    elseif spell == "Razor Wind" then 
        ...
        for i = 0, 3 do
            ...
            addEvent(doMoveInArea2, i*300, cid, t[d][1], areaEff[i+1], NORMALDAMAGE, 0, 0, spell) -- s? o effect
            addEvent(doMoveInArea2, i*300, cid, 0, areaDano[i+1], NORMALDAMAGE, min, max, spell) -- s? o dano
        end
...
--[[    
    ...
    elseif isInArray({"Earthshock", "Earth Power", "Frost Power", "Iceshock"}, spell) then -- fazer recolor, groundshock do crystal onix est? igual o iceshock
    ...
    elseif isInArray({"Restore", "Selfheal", "Recover", "Softboiled",}, spell) then -- ver sunny day, rain dance e outros com b?nus de cura
...
--      doMoveInArea2(cid, 114, reto5, POISONDAMAGE, min, max, spell)
    ...
    elseif spell == "Toxic" then -- colocar um pouco de transpar?ncia no effect 114 (ou add outro igual pra por (s? pro t?xic))
    ...
    elseif spell == "Poison Gas" then 
        ...
        -- local ret2 = {id = 0, cd = 13, check = 0, damage = dmg, cond = "Poison"} -- funciona o poison se o poke estiver no target, sem target n?o funciona --rever isso ainda!!
    ...
    elseif spell == "Dynamic Punch" then -- chance de causar stun
        ...
        addEvent(doMoveDano2, 50, cid, target, FIGHTINGDAMAGE, 0, 0, ret, spell) -- condi??o(stun)
    ...
    elseif spell == "Revenge" then -- Refeito igual PxG, Sam. ( tentar fazer o c?digo de outra forma, com for 0,4 do eff+1 )
    ...
    elseif spell == "Psy Pulse" or spell == "Cyber Pulse" or spell == "Dark Ball" then -- conforme o anime /\
        ...
        --if not isInArray({"Absol", "Mega Absol"}, getSubName(cid, target)) then -- colocar s? para o MEGA (pegando mega id)
    ...
    elseif spell == "Rock Throw" then -- editar, colocar pequena chance de burn se for pedra de fogo, fazer passiva pra isso
        ...
        local atk = { -- missile, effect
            ["Rock Throw"] = {11, 44, 0, 176, 70, 716}, -- padr?o, crystal, lava
        }
    ...
    elseif spell == "Rock Slide" then -- editar, colocar pequena chance de burn se for pedra de fogo, fazer passiva pra isso
        -- EDITAR atk = para local atk = (tava dando bug aqui, tem v?rias spells que n?o ta como local...)
        
        local atk = { -- missile, effect
            ["Rock Slide"] = {11, 44, 0, 176, 70, 716}, -- padr?o, crystal, lava
            --Stone Edge"] = {11, 239}
        } 
        if isInArray({"Numel", "Shiny Numel", "Camerupt", "Shiny Camerupt", "Mega Camerupt", "Magcargo", "Shiny Magcargo", "Slugma", "Shiny Slugma"}, getSubName(cid, target)) then
            effD = atk[spell][5]
    ...
    elseif spell == "Rest" then -- criar aquela passiva do slaking que faz dormir s? que n?o cura a vida (newStatusSyst l? pela linha 671)
        ...
        if team[name] then
            ...
            for b = 2, num do
                ...
                if pokeSourceCode == "Nao" then
                    doSendMsg(master, "N?o h? espa?o para seu pokemon.")
                    return true 
                end  
    ...
    elseif spell == "Guillotine" then -- editar/refazer effect, usar pinsir skill outfit pra pegar anima??o dos chifres dele 
    ...
    elseif isInArray({"Dragon Mist", "Mega Wing", "Mach Punch", "Nuzzle", "Shadow Sneak", "Shadow Mist", "Flash Kick", "Fenix Dash"}, spell) then 
        ...
        if spell == "Shadow Sneak" then
            ...
            doMoveInArea2(cid, 0, triplo6, qualDano, min, max, spell) -- area padr?o para o resto da sspells (6 sqm de dist?ncia e 3 sqm de largura)
            doSendMagicEffect(t[a][2], t[a][1])
        end
    ...
    elseif spell == "Muddy Water" then 
        ...
        if isInArray({"Quagsire", "Swampert", "Whiscash"}, getSubName(cid, target)) then
            ...
            doMoveInArea2(cid, 844, muddy, GROUNDDAMAGE, min, max, spell, ret) -- colocar transpar?ncia (116), fazer um parecido marrom
        end 
    ...
    elseif spell == "Invisible" or spell == "Instant Teleportation" then
        ...
        setPlayerStorageValue(cid, 9658783, 1) -- n?o levar dano

        if spell == "Instant Teleportation" then
            doSendMagicEffect(getThingPosWithDebug(cid), 134)       
        else    
            doSendMagicEffect(getThingPosWithDebug(cid), 134) -- 386 no spr do dxp esp (conferir qual eff ?)
        end
...
elseif spell == "Dark Eye" or spell == "Miracle Eye" or spell == "Dark Accurate" then -- fazer/editar dark accurate eff 725
    ...
    elseif spell == "Stampade" or spell == "Stampede" or spell == "Crow Swarm" then -- se o nome for outro pra spell(Murkrow) elseif spell == "OutaSpell" then docastspell(cid, "Stampade")
        ...
        for rocks = 1, 42 do
            if getSubName(cid, target) == "Tauros" then -- n?o tem o effect no client, usando do shiny tauros
    ...
    elseif spell == "Air Cutter" then -- lembrar do atk do mega absol do pxg com os cortes brancos mais rapidos
        ...
        for a = 0, 5 do
            ...
            local t = {  -- Effect corte em dire??o Desativado
    ...
    elseif spell == "Crunch" then
        ...
        addEvent(doDanoWithProtect, 10, cid, DARKDAMAGE, posT1, 0, 0, 0, 536) -- s? effect
        addEvent(doDanoWithProtect, 10, cid, DARKDAMAGE, posT1, 0, 0, 0, 535) -- s? effect
        addEvent(doDanoWithProtect, 10, cid, DARKDAMAGE, posT1, 0, 0, 0, 536) -- s? effect
    ...
    elseif spell == "Leech Life" then --refazer, n?o ta curando. Fazer um effect de "sugar" do target
    ...
    elseif spell == "Metal Claw" then
        ...
        posT1.y = posT1.y-1 -- somente x = x+1 (padr?o do posT1 ? x+1 e y+1)
    ...
    elseif isInArray({"Croak Hook", "Tongue Hook", "Vine Hook"}, spell) then -- Sam
        ...
        local Shoot = 38 -- effect vermelho(padr?o dos sapos)
        ...
        if spell == "Vine Hook" then
            ...
            elseif getSubName(cid, target) == "Tangela" then
                ...
                local Shoot = 38 -- effect verde(padr?o vine hook)
            end 
        end
    ...
    elseif isInArray({"Psy Impact", "Sky Attack", "Discharge", "Ice Spikes", "Eruption Terrain", "Fairy Burst", "Mega Discharge", "Poison Burst", "Ground Elevation", "Flames", "Storm Leaves", "Lava Pool", "Heart Stamp", "Sand Power", "Cold Storm"}, spell) then
        ...
        elseif spell == "Flames" then
            dano = FIREDAMAGE
            eff = 505
            --  ret.cond = "Burn" -- Burn (ainda n?o funciona)
        ...
        elseif spell == "Storm Leaves" then
            dano = GRASSDAMAGE
            dano2 = FLYINGDAMAGE --por enquanto s? ta ativo 1 dano (n?o sei se existe poke de planta e voador, e o eff de flying ainda vou trocar)          
            eff = 694   
            eff2 = 643
        ...
        elseif spell == "Ground Elevation" then
            ...
            addEvent(doMoveInArea2, 500, cid, 0, BigArea2, dano, min, max, spell, ret) -- padr?o
        end 
    ...
    elseif spell == "Wild Charge" then -- fazer um ataque dark igual esse s? q com effect 679
        
        eff2 = 48
        if isInArray({"Shiny Electabuzz", "Shiny Electivire"}, getSubName(cid, target)) then
            eff2 = 641
        elseif isInArray({"Shiny Lanturn", "Shiny Magneton"}, getSubName(cid, target)) then 
            eff2 = 979 -- fazer um ataque dark igual esse s? q com effect 679
        end 
    ...
    elseif spell == "Focus Blast" then -- refazer, no anime lan?am uma bola azul, exceto machoke, lan?a um kamehameh? amarelo
    ...
    elseif spell == "Scald" then 
        ...
        doMoveInArea2(cid, 396, flamek, WATERDAMAGE, min, max, spell) -- fuma?a do scald
        doSendMagicEffect(flamepos, effect) 
        doBurnPoke(cid, target)
    ...
    elseif spell == "Song Burst" then
        ...
        local areas = {rock1, rock2, rock3, rock4, rock5, rock1, rock2, rock3, rock4, rock5} -- fiz ao contr?rio do wild charge
    ...
    elseif spell == "Poison Shock" then --dano em reta3, arrumar, criar outra. usar de base pra varias spells, plantas e pedras saindo do ch?o
        ...
        function sendAtk(cid, area, areaEff, eff)
            ...
            if isCreature(cid) then
                ...
                doAreaCombatHealth(cid, POISONDAMAGE, area, whirl3, -min, -max, 105) --542(plight) nn sei se esse ? o effect nem se a spell ? assim no PxG 
        ...
        -- Fazer Spells de Vento sem usar furac?es...
    ...
    elseif spell == "Ice Ball" then -- Pxg s? falta deixar o Paralyze funcionando. Vou fazer um missile(bola de gelo tbm) , melhor fazer uma pequena chance de congelar, tentar usar outro effect menor de congelado
        ...
        addEvent(doMoveDano2, 195, cid, target, NORMALDAMAGE, 0, 0, ret, spell) -- s? pra aplicar o paralyze (sem dano nem efeito)
    ...
    elseif spell == "Lightning Horn" then -- Deixei com dano em area, n?o tenho ctz 
        local pos = getThingPosWithDebug(target)
    ...
    elseif spell == "Bullet Seed" then
        ...
        for i = 1, Sparks do -- solta de 2 a 9 shots. ("for i =1, 2 do" = 2shots) jay sempre ser? no M?nimo 2
            
            addEvent(doIce, i*350, cid, target) --300 
        end
        
        
        -- j? tem o missile no spr do plight, trocar dps.
    elseif spell == "Ice Sparks" then -- vou criar um missile(fagulhas de gelo) p/ a spell. Falta arrumar o effect nos types
        -- condi??o do effect padr?o do ICEDAMAGE
        local function doIce(cid, target) 
            if not isCreature(cid) or not isCreature(target) then return false end
    ...
    elseif spell == "Ice Shards" then -- SPELL BASE, solta 2 shoots
        ...
        -- refazer, usar overheat maneiro com esse effect de fuma?a
    elseif spell == "Smog" then -- Causa Slow e Poison, por?m poison ainda n?o funciona(ret) e n?o sei por 2 debuff(ret)
        ...
        local function doQuake(cid)
            ...
            local ret2 = {} -- improvisei, n?o testei ainda, coloquei outra function e outro "ret2" sem dano nem eff, s? pra dar poison
            ret.id = 0
            ret.cd = 9
            ret.eff = 0
            ret.check = 0
    ...
    elseif spell == "Static Shot" then -- sparkline
        ...
        if a == 1 or a == 2 then
            ...
            doMoveInArea2(cid, t[a], reto1, ELECTRICDAMAGE, 0, 0, spell) -- s? o effect 
        end
    ...
    elseif spell == "Meditate" then -- fica imune e im?vel por 3 segundos, no fim da execu??o ele remove status negativos
    ...
    elseif spell == "Shed Skin" then -- Faz com que o pok?mon cure os status negativos
    ...
    elseif spell == "Hex" then -- fica imune e im?vel por 3 segundos, no fim da execu??o ele remove status negativos
    ...
    elseif spell == "Static Field" then -- impedir que durmam caso estejam na area eletrizada, e caso estejam dormindo ser?o acordados
        ...
        local function smoke(cid)
            ...
            if isSleeping(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return false end
            if isWithFear(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return true end
            doMoveInArea2(cid, 433, electricTerrain3, ELECTRICDAMAGE, 0, 0) -- corresponde ? area confusion, S? PARA O EFFECT
            doMoveInArea2(cid, 0, confusion, ELECTRICDAMAGE, 0, 0, spell, ret) -- s? para o Paralyze(ret)
        end
    ...
    elseif spell == "Drain Punch" then
        ...
        -- fazer passiva Illusion , shredder team passivo, usar n?mero aleat?rio de clones, misdreavus tem a passiva e mais alguns ai.
    ...
    elseif spell == "Leaf Storm" then
        local eff = {245, 497, 507, 245, 496, 165} --245 497 480 307(fuma?a)
        local eff2 = {167, 245, 168, 245} -- 167(aura furac?o) 168(aura folha) 165(aura cinza)
    ...
    elseif spell == "Psycho Cut" then -- Psycho Cut
        ...
        -- /\ esse 98 ? nulo, pq se eu colocar "0" ele pega o missile de n? 1
...
elseif spell == "Helping Hand" then 
    ...
    elseif spell == "Draining Kiss" then -- effect kiss, dps effect de buff vermelho(rage?) causa dano em grande area e cura conforme o dano e numero de pokemons acertados(essa parte ainda n?o sei fazer)
    ...
    elseif spell == "Meteor Strike" then 
        ...
        local function doFall(cid)
            ...
            for rocks = 1, 42 do --62 -- 140
                addEvent(fall, rocks*35, cid, master, FIREDAMAGE, 98, 538) -- 702 ? um pouco mais vermelho/escuro
...
--      addEvent(setPlayerStorageValue, 1400, cid, 32698, -1) -- storage do silence
    ...
    elseif spell == "Stone Edge" then
        ...
        atk = { -- missile, effect
            ["Stone Edge"] = {124, 44}, -- padr?o, crystal, lava
        }
    ...
    elseif spell == "old bola de canh?o" then 
    ...
    elseif spell == "Mud Sport" then -- v?rios danso consecutivos e blind (math.random)
    ...
    elseif spell == "Mirror Shot" then -- sparkline
        ...
        if a == 0 or a == 3 then
            doMoveInArea2(cid, t[a], mirrorShot1, STEELDAMAGE, 0, 0, spell) -- s? effect
        else
            doMoveInArea2(cid, t[a], mirrorShot2, STEELDAMAGE, 0, 0, spell) -- s? effect 
        end
...
-- se tiver com storage de silence, n?o causa dano a spell, tipo, essa causa dano 2 vezes, ent, tem q setar -1 antes do doMoveInArea2 , dps seta silence dnv e antes do prox dano seta -1 novamente
...
eff = 64
if a == 0 then -- affastar o effects pra l? << um pouco
doSendMagicEffect({x=p.x+1, y=p.y-1, z=p.z}, 299)
eff = 64
elseif a == 1 then
eff = 65
doSendMagicEffect({x=p.x+5, y=p.y+1, z=p.z}, 296)
elseif a == 2 then  -- affastar o effects pra l? << um pouco
eff = 66
...
elseif spell == "Air Twister" then 
    ...
    elseif spell == "Static" then
        ...
        local function doStick(cid)
            ...
            posC2.x = posC2.x+1
            if isInArray({"Shiny Electabuzz", "Shiny Electivire"}, getSubName(cid, target)) then
                eff = 539 -- azul. Tem o 642 q ? preto com azul
            else
