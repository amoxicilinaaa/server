...
function addMixlortPokeXp(cid, target)
    ...
    if isPlayer(target) == FALSE and getItemAttribute(feet.uid, "level") < 100 and ((XPMixlort*2) + exp) >= (level * 20) + (20 *(level - 1)) then
        ...
        doCreatureSay(cid, getPokeName(getCreatureSummons(cid)[1])..", você esta ficando muito forte!", TALKTYPE_SAY)    
...
local ret = {}
-- if not PokeInfoPortrait[name] then 
-- local nullfoto = 2395
-- table.insert(ret, nullfoto.."")
-- end
if not PokeInfoPortrait[name] then return true end
if PokeInfoPortrait[name] >= 1 then
local foto = PokeInfoPortrait[name] or 2395
...
local storages = {  -- Todas as storages usadas em quaisquer scripts ter?o q ser armazenadas aqui
    iconSys = 20000,
    pokedexSys = 8052,
