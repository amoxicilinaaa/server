...
if not isPlayer(cid) then--or isInArray({5, 6}, getPlayerGroupId(cid)) then --alterado v1.9
return true
end
...
if isPlayer(cid) and getCreatureOutfit(cid).lookType == 814 then return false end
	...
	if not isInArray(waters, getTileInfo(getThingPos(cid)).itemid) then
		...
		local x = surfs[pokemon]

		if not x then return true end

		if getItemAttribute(item.uid, "nome") then
			doCreatureSay(cid, getItemAttribute(item.uid, "nome")..", Cansei de surfar!", 1)
		else
			doCreatureSay(cid, getItemAttribute(item.uid, "poke")..", Cansei de surfar!", 1)
		end
