function onCastSpell(cid, var)
    ...
    elseif spell == "Discharge" then
        dano = ELECTRICDAMAGE
        eff = isInArray({"Shiny Electabuzz", "Shiny Electivire"}, name) and 640 or 409
        ret.cond = "Miss"
        ret.eff = 541
    ...
    elseif spell == "Fairy Burst" then
        dano = NORMALDAMAGE -- alterar para FAIRYDAMAGE se disponível
        eff  = 644
    ...
    -- Execução de múltiplos impactos com efeito visual
    for rocks = 1, 20 do
    ...
    -- Execução de dano em área com delay
    if spell == "Storm Leaves" then
        addEvent(doMoveInArea2, 500, cid, 0, BigArea2, dano2, 0, 0, spell, ret)
    ...
    elseif spell == "Ground Elevation" then -- duplicado, pode ser removido
