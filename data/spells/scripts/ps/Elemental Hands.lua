...
local movestable = getTableMove(cid, spell) --alterado v1.6
    ...
    if not isPlayer(cid) then
        if type(movestable) == 'table' and movestable.f ~= 0 then   --alterado v1.6
    ...
    --- FOCUS ----------------------------------            
    if getPlayerStorageValue(cid, 253) >= 0 and type(movestable) == 'table' and movestable.f ~= 0 then   --alterado v1.6
...
if getPlayerStorageValue(cid, 999457) >= 1 and type(movestable) == 'table' and movestable.f ~= 0 then    --alterado v1.6
    ...
    ---------------------fim da padrão /\ e aparti daqui \/ é o code da spell------------------------------
		...
		if getCreatureOutfit(cid).lookType == 1301 then
			print("Error aconteceu com o move 'Elemental Hands', a outfit do hitmonchan esta incorreta, reporte isso para a equipe um Administrador do Pokemon HP")
