...
["Bagon"] = {crystal},
["Shelgon"] = {crystal},
["Salamence"] = {crystal, heart},
["Slaking"] = {heart},

["Mamoswine"] = {ice, earth},
["Regirock"] = {rock},
["Shiny Kingdra"] = {water, crystal},
["Shiny Tyranitar"] = {rock, dark},
["Unown"] = {enigma},
["Sceptile"] = {leaf},
["Electivire"] = {thunder},
["Magmortar"] = {fire, magma},
["Lucario"] = {punch, metal},
["Riolu"] = {punch},
["Rayquaza"] = {heart, crystal},
["Togetic"] = {heart},
["Azumarill"] = {water},
["Torterra"] = {leaf, earth},
["Empoleon"] = {water, metal},
...
function onUse(cid, item, topos, item2, frompos)
    ...
    if item.itemid == button then
        ...
        local myball = getDepotItem(ball_slot.uid, 0, cid, 3)
		if not myball then
			doPlayerSendCancel(cid, "The item does not exist or has been removed. Envie esse erro no discord para nosso administrador")
			return true
		end
		if myball.uid == 0 then
			doPlayerSendCancel(cid, "Você precisa colocar uma pokebola na máquina")
		return true
		end
		
		local memoryDittoShiny = getItemAttribute(myball.uid, "ehshinyditto")
		local memoryDitto = getItemAttribute(myball.uid, "ehditto")
		local pokeNameS = getItemAttribute(myball.uid, "poke")

		if memoryDittoShiny and pokeNameS ~= "Shiny Ditto" then
		    doPlayerSendTextMessage(cid, 27, "Destransforme seu Shiny Ditto primeiro!")
			return true
		elseif memoryDitto and pokeNameS ~= "Ditto" then
			doPlayerSendTextMessage(cid, 27, "Destransforme seu Ditto primeiro!")
			return true
		end
        ...
        local stone = getDepotItem(stone_slot.uid, 0, cid, 2)--getContainerItem(stone_slot.uid, 0)  
        if stone.uid <= 0 then
        ...
        for a = 0, 10 do
            local it = getDepotItem(stone_slot.uid, a, cid, 2)--getContainerItem(stone_slot.uid, a)
            if it.uid > 0 then
...
--print("required_stones "..required_stones)
--print("boosts "..boosts)
--boosts = 1
--removeStones = 1
        ...
        for b = 1, removeStones do
            local i = getDepotItem(stone_slot.uid, 0, cid, 2)--getContainerItem(stone_slot.uid, 0)
        ...
        --addEvent(setDepotIdMachine, 3000, ball_slot, stone_slot)  
		return true
    end
end