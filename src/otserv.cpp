...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version")
		{
			std::cout << STATUS_SERVER_NAME << " (" << STATUS_SERVER_CODENAME << "), last compiled at " << __DATE__ << ".\n"
...
void serverMain(void* param)
{
	...
	sigaction(SIGINT, &s, NULL); //ctrl+c
    #endif
	#endif
...
ServiceManager* services)
{
    ...
    std::cout << STATUS_SERVER_NAME << " (" << STATUS_SERVER_CODENAME << "), last compiled at " << __DATE__ << "." << std::endl;
	std::cout << "Credits to all TFS team (Elf, slawkens, Talaturen, Lithium, KaczooH, Kiper, Kornholijo)." << std::endl;
	std::cout << "Pokemon MS distro" << std::endl << std::endl;
	...
	if(GetLastError() == ERROR_ALREADY_EXISTS)
		startupErrorMessage("Another instance of this server is already running with the same worldId.\nIf you want to run multiple servers, please change the worldId in configuration file.");
	...
	//pokeaddon
	std::cout << ">> Loading pokemon addons" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Loading pokemon addons");
	#endif
	if(!PokeAddons::getInstance()->loadFromXml())
		startupErrorMessage("Unable to load pokemon addons!");
			...
			switch(LOWORD(wParam))
			{
				...
				case ID_MENU_MAIN_CLEARLOG:
				{
					if(g_game.getGameState() != GAME_STATE_STARTUP)
					{
						...
						std::cout << STATUS_SERVER_NAME << " (" << STATUS_SERVER_CODENAME << "), last compiled at " << __DATE__ << "." << std::endl;
						std::cout << "Credits to all TFS team (Elf, slawkens, Talaturen, Lithium, KaczooH, Kiper, Kornholijo)." << std::endl;
						std::cout << "Pokemon MS distro" << std::endl;
						std::cout << std::endl;
					}
