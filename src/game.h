...
class Game
{
	public:
		...
		void setPvpArenaEnemyList(Player* Enemy)
		{
			pvpArenaEnemyList.insert(Enemy);
		}
		bool removeEnemyList(Player* _guid)
		{
		    ArenaEnemySet::iterator it = pvpArenaEnemyList.find(_guid);
			if(it == pvpArenaEnemyList.end())
				return false;

			pvpArenaEnemyList.erase(it);
			return true;
		}
		...
		ReturnValue placeSummon(Creature* creature, const std::string& name, const std::string& nick, bool forced = false, Position* pos = NULL);
		...
		void onUpdateFlyTile(Position pos);
		...
		/**
		  ...
		  */
		uint64_t getMoney(const Cylinder* cylinder);//viktorgold
		...
		/**
		  ...
		  * \returns true if the removal was successful
		  */
		bool removeMoney(Cylinder* cylinder, int64_t money, uint32_t flags = 0);//viktorgold
		...
		/**
		  ...
		  */
		void addMoney(Cylinder* cylinder, int64_t money, uint32_t flags = 0);//viktorgold
		void addMoneyMarket(std::string name, int64_t money);
		bool removeMoneyMarket(Player* player, int64_t money);
		...
		bool playerMoveThing(uint32_t playerId, const Position& fromPos, uint16_t spriteId,
			int16_t fromStackpos, const Position& toPos, uint16_t count);//65kitem
		...
		bool playerLookAt(uint32_t playerId, const Position& pos1, uint16_t spriteId, int16_t stackpos);
		...
		bool pokeChangeOutfit(uint32_t playerId, Outfit_t outfit);
		bool playerInviteToParty(uint32_t playerId, uint32_t invitedId);
		//duel system
		bool playerInviteToDuel(uint32_t playerId, uint32_t invitedId);
		bool playerOkDuel(Player* player, Player* invitedPlayer);
		bool playerCancelDuel(Player* player);
		bool playerJoinDuel(uint32_t playerId, uint32_t leaderId);
		bool playerRevokeDuelInvitation(uint32_t playerId, uint32_t invitedId);
		bool playerLeaveDuel(uint32_t playerId);
		//fim
		...
		//Market
		bool requestSellMarket(uint32_t playerId, const Position& pos, int16_t stackpos, uint16_t spriteId);
		bool acceptSellMarket(uint32_t playerId, uint16_t count, uint64_t price, bool type_money, bool type_unity);
		bool playerCloseMarket(uint32_t playerId);
		...
		void internalCreatureChangePokeOutfit(Creature* creature, const Outfit_t& oufit, bool forced = false);
