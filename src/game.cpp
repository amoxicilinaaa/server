...
void Game::setGameState(GameState_t newState)
{
	...
	{
		std::cout << "GAME_STATE_SHUTDOWN" << " desligando." << std::endl;
	...
	if(gameState != newState)
	{
		...
		switch(newState)
		{
			case GAME_STATE_INIT:
			{
				...
				IOMarket::getMarket()->loadItemsMarket();
				IOMarket::getMarket()->loadXmlMarket();
			...
			{
			    std::cout << "GAME_STATE_NORMAL" << " desligando." << std::endl;
				break;
			}
			case GAME_STATE_MAINTAIN:
			{
			    std::cout << "GAME_STATE_MAINTAIN" << " desligando." << std::endl;
				break;
			}
			case GAME_STATE_STARTUP:
			{
			    std::cout << "GAME_STATE_STARTUP" << " desligando." << std::endl;
				break;
			}
			case GAME_STATE_CLOSING:
			{
		        std::cout << "GAME_STATE_CLOSING" << " desligando." << std::endl;
				break;
		    }
			default:
				break;
	...
	std::cout << "GAME_C" << " desligando." << std::endl;
}

void Game::saveGameState(bool crash /*= false*/) // Monarca Save
{
    ...
    IOMarket::getMarket()->saveItemsMarket();
...
ReturnValue Game::placeSummon(Creature* creature, const std::string& name, const std::string& nick, bool forced, Position* pos)
{
...
bool Game::playerMoveThing(uint32_t playerId, const Position& fromPos,
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint16_t count)//65kitem
{
...
ReturnValue Game::internalMoveCreature(Creature* creature, Direction direction, uint32_t flags/* = 0*/)
{
	...
	bool deny = false;
    CreatureEventList moveEvents = creature->getCreatureEvents(CREATURE_EVENT_WALK);
    for(CreatureEventList::iterator it = moveEvents.begin(); it != moveEvents.end(); ++it)
       if(!(*it)->executeWalk(creature, currentPos, destPos))
           deny = true;
		...
		if(thing)
		{
			...
			if(thing->getItem() && toItem->getID() >= 1)
			{
				if(toItem->getID() == 460 || toItem->getID() == 11676 || toItem->getID() == 11675 || toItem->getID() == 11677)
				{
					...
					onUpdateFlyTile(fromTile->getPosition());
				}
			}
		}
    }
		...
		if(Player* player = creature->getPlayer())
		{
			...
			if((player->getStorage(17000, value) && atoi(value.c_str()) >= 1) && !map->getTile(destPos))
			{
				...
				if (!checkTile->hasProperty(BLOCKSOLID) && !checkTile->ground)
                {
                    ...
                    if(map->getTile(fromTile->getPosition()) && retF == RET_NOERROR)
                    {
						Tile* tileFFly = getTile(fromTile->getPosition());
						Thing* thing = tileFFly->__getThing(0);
					    if(thing)
	...
	if(Player* player = creature->getPlayer())
	{
		...
		// if(playerLookType == 2191)
		// 	effect = 1031;
		// else if(playerLookType == 2192)
		// 	effect = 1031;

		// else if(playerLookType == 2189)
		// 	effect = 1032;
		// else if(playerLookType == 2190)
		// 	effect = 1032;

		// else if(playerLookType == 2139)
		// 	effect = 1030;
		// else if(playerLookType == 2140)
		// 	effect = 1030;

		// else if(playerLookType == 2195)
		// 	effect = 1029;
		// else if(playerLookType == 2196)
		// 	effect = 1029;

		// else if(playerLookType == 2259)
		// 	effect = 615;
		// else if(playerLookType == 2260)
		// 	effect = 615;
...
void Game::onUpdateFlyTile(Position pos)
{
	const Position& cylinderMapPos = pos;

	const SpectatorVec& list = getSpectators(cylinderMapPos);
	...
	for(it = list.begin(); it != list.end(); ++it)
	{
		if((tmpPlayer = (*it)->getPlayer()))
		{
			if((tmpPlayer->getStorage(17000, flySto) && atoi(flySto.c_str()) >= 1))
			tmpPlayer->sendUpdateTileFly(pos);
		}
	}
}
	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint16_t count) //65kitem
{
	...
	if(toPos.x == 65535 && toPos.y == 8)
	{
        Thing* athingM = player->__getThing(8);
        if(athingM && athingM->getItem())
        {
            boost::any abMBtM = athingM->getItem()->getAttribute("moveBallT");
	        if(abMBtM.type() == typeid(std::string))
	        {
	 	        std::string apBMBtM = boost::any_cast<std::string>(abMBtM);
	 	        if(apBMBtM == "use")
	 	        {
	...
	//market
	boost::any valuemarket = (item)->getAttribute("market");
	if(!valuemarket.empty())
	     (item)->eraseAttribute("market");

	if(fromPos.x == 65535 && fromPos.y == 7 || toPos.x == 65535 && toPos.y == 7)
    {
	    player->sendTextMessage(MSG_INFO_DESCR, "sorry, not possible.");
 	 	return false;
    }
	...
	boost::any bMBt = (item)->getAttribute("moveBallT");
	if(bMBt.type() == typeid(std::string))
	{
	 	std::string pBMBt = boost::any_cast<std::string>(bMBt);
	 	if(pBMBt == "use")
	 	{
	 	    player->sendTextMessage(MSG_INFO_DESCR, "sorry, not possible.");
	 	    return false;
	...
	//char buffer[254];
		               // sprintf(buffer, " priguito %d", item->getTopParent()->getItem()->getID());
					  // if(item->getHoldingPlayer())
					//	   player->sendTextMessage(MSG_INFO_DESCR, "priquito doce");
					  // else
					//	   player->sendTextMessage(MSG_INFO_DESCR, "priquito azedo");
	//player->sendTextMessage(MSG_INFO_DESCR, buffer);
	if(fromPos.x != 65535 && toPos.x == 65535)
	{
		Container* contaBar = item->getContainer();
		uint8_t cBallbar = player->getMana();

		boost::any valueP = (item)->getAttribute("poke");
		bool vai = true;
		if(!contaBar && cBallbar < 6 && !valueP.empty() && valueP.type() == typeid(std::string))
		{
			cBallbar +=1;
			(item)->setAttribute("ballorder", cBallbar);//aqui
			vai = false;
	    }
		else if(contaBar)
		{

			for(ContainerIterator itBar = contaBar->begin(), end = contaBar->end(); itBar != end; ++itBar)
      		{
				if(cBallbar >= 6)
				{
					vai = true;
				...
				valueP = (*itBar)->getAttribute("poke");
		    	if(!valueP.empty() && valueP.type() == typeid(std::string))
		    	{
					cBallbar +=1;
					(*itBar)->setAttribute("ballorder", cBallbar);
					vai = false;
				}
			}
	...
	}else if(toPos.x == 65535 && !item->getHoldingPlayer())
	{
		int8_t bo = 0;
		if(player->getMana() && toPos.y == 8)
			bo = -1;
        ...
        item->setAttribute("ballorder", bo+1);
		// player->sendTextMessage(MSG_INFO_DESCR, "priquito linsooo");
		player->doUpdatePokemonsBar();
	}

	boost::any valueU = (item)->getAttribute("unico");
	if(valueU.type() == typeid(int32_t))
	{
        if(boost::any_cast<int32_t>(valueU) == 1)
        {
			Tile* tileUnicMap = getMap()->getTile(fromPos);
			if(tileUnicMap)
			{
				HouseTile* houseMapUnic = tileUnicMap->getHouseTile();
		        if(houseMapUnic)
				{
		            House* houseUnicMap = houseMapUnic->getHouse();
				    if(houseUnicMap)
					{
						House* houseOUnicMap = Houses::getInstance()->getHouse(houseUnicMap->getId());
						/*std::string nameDono; //Unico para a conta toda
						uint32_t idDonoHouse;
						IOLoginData::getInstance()->getNameByGuid(houseOUnicMap->getOwner(), nameDono, 0);
						IOLoginData::getInstance()->getAccountId(nameDono, idDonoHouse);
						if(player->getAccount() != idDonoHouse)*/
						if(player->getGUID() != houseOUnicMap->getOwner())
						{
							player->sendTextMessage(MSG_INFO_DESCR, "It is a Unique Item, you can't throw away this item!");
			    			return false;
						}
					}
				}
			...
			Tile* tileUnic = getMap()->getTile(toPos);
            if(tileUnic)
			{
		        HouseTile* houseTileUnic = tileUnic->getHouseTile();
		        if(houseTileUnic)
				{
		            House* houseUnic = houseTileUnic->getHouse();
				    if(houseUnic)
					{
						House* houseOUnic = Houses::getInstance()->getHouse(houseUnic->getId());
						/*std::string nameDono; //Unico para a conta toda
						uint32_t idDonoHouse;
						IOLoginData::getInstance()->getNameByGuid(houseOUnic->getOwner(), nameDono, 0);
						IOLoginData::getInstance()->getAccountId(nameDono, idDonoHouse);
						if(player->getAccount() != idDonoHouse)*/
						if(player->getGUID() != houseOUnic->getOwner())
						{
							player->sendTextMessage(MSG_INFO_DESCR, "It is a Unique Item, you can't throw away this item!");
			    			return false;
						}
            ...
            }else if(thingT && thingT->getItem())
            {
			    Item* itemT = thingT->getItem();
			    if(!itemT->isDepot() && itemT->isContainer() && !itemT->getHoldingPlayer())//itemT->getID() != 1987)
				{
					player->sendTextMessage(MSG_INFO_DESCR, "It is a Unique Item, you can't throw away this item!");
			    	return false;
				}
                ...
                {
                    Item* itemTC = toCylinder->getItem();
                    if(!itemTC->isDepot() && itemTC->isContainer() && !itemTC->getHoldingPlayer())//itemTC->getID() != 1987)
				    {
					    player->sendTextMessage(MSG_INFO_DESCR, "It is a Unique Item, you can't throw away this item!");
			            return false;
				    }
    ...
    if(item->getContainer())
	{
		...
		for(ContainerIterator sb = container->begin(), end = container->end(); sb != end; ++sb)
		{
            ...
            boost::any valueU = (ballSLot)->getAttribute("unico");
	        if(valueU.type() == typeid(int32_t))
	        {
                if(boost::any_cast<int32_t>(valueU) == 1)
                {
	...
	Thing* thingT = internalGetThing(player, toPos, toIndex, spriteId, STACKPOS_MOVE);
	if(thingT && thingT->getItem() && (toPos.y == 8 || toPos.y == 10))
	{
		if(!item->getHoldingPlayer())
		{
			Item* itemT = thingT->getItem();
		    valueU = (itemT)->getAttribute("unico");
		    if(valueU.type() == typeid(int32_t) && boost::any_cast<int32_t>(valueU) == 1)
	        {
			    player->sendTextMessage(MSG_INFO_DESCR, "It is a Unique Item, you can't throw away this item!");
			    return false;
		    }
	...
	if(thingT && thingT->getItem()) //viktor
	{
		Item* toItemU = thingT->getItem();
		if(item->isStackable())
		{
			if(toItemU->getID() == item->getID())
			{
				boost::any vUi = (item)->getAttribute("unico");
				boost::any vUTi = (toItemU)->getAttribute("unico");
				if(vUi.empty() && !vUTi.empty() || !vUi.empty() && vUTi.empty())
	        	{
					player->sendTextMessage(MSG_INFO_DESCR, "It is a Unique Item, you can't throw away this item!");
					return false;
				}
	...
	int32_t index, Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/)
{
	...
	if(item->isStackable())
	{
		...
		{
			n = std::min((uint32_t)1000 - toItem->getItemCount(), m);//65kitem
	...
	uint32_t flags /*= 0*/, bool test /*= false*/)
{
	...
	if(!test)
	{
		...
		{
			uint32_t n = std::min((uint32_t)1000 - toItem->getItemCount(), m);//65kitem
			toCylinder->__updateThing(toItem, toItem->getID(), toItem->getItemCount() + n);
			if(m - n > 0)
			{
...
uint64_t Game::getMoney(const Cylinder* cylinder)//viktorgold
{
	if(!cylinder)
		return 0;
	...
	uint64_t moneyCount = 0;//viktorgold
...
bool Game::removeMoney(Cylinder* cylinder, int64_t money, uint32_t flags /*= 0*/)//viktorgold
{
	if(!cylinder)
		return false;
	...
	int64_t moneyCount = 0;//viktorgold
	...
	for(MoneyMultiMap::iterator mit = moneyMap.begin(); mit != moneyMap.end() && money > 0; ++mit)
	{
		...
		if(mit->first > money)
		{
			...
			int64_t remaind = item->getWorth() - money;//viktorgold
...
void Game::addMoney(Cylinder* cylinder, int64_t money, uint32_t flags /*= 0*/)//viktorgold
{
	IntegerMap moneyMap = Item::items.getMoneyMap();
	int64_t tmp = 0;//viktorgold
	for(IntegerMap::reverse_iterator it = moneyMap.rbegin(); it != moneyMap.rend(); ++it)
	{
		...
		if(tmp != 0)
		{
			do
			{
				Item* remaindItem = Item::CreateItem(it->second, std::min((int64_t)100, tmp));//viktorgold
				...
				tmp -= std::min((int64_t)100, tmp);//viktorgold
			}
			while(tmp > 0);
		}
	}
...
void Game::addMoneyMarket(std::string name, int64_t money)
{
	IntegerMap moneyMap = Item::items.getMoneyMap();
	int64_t tmp = 0;
	for(IntegerMap::reverse_iterator it = moneyMap.rbegin(); it != moneyMap.rend(); ++it)
	{
		...
		if(tmp != 0)
		{
			do
			{
				Item* remaindItem = Item::CreateItem(it->second, std::min((int64_t)100, tmp));
				IOLoginData::getInstance()->playerNewMail(NULL, name, 1, remaindItem);
...
bool Game::removeMoneyMarket(Player* player, int64_t money)
{
	if(!player)
		return false;
	...
	typedef std::multimap<int64_t, Item*, std::less<int64_t> > MoneyMultiMap;
	MoneyMultiMap moneyMap;
	...
	for(int32_t i = player->__getFirstIndex(); i < player->__getLastIndex() && money > 0; ++i)
	{
		if(!(thing = player->__getThing(i)))
			continue;
	...
	for(MoneyMultiMap::iterator mit = moneyMap.begin(); mit != moneyMap.end() && money > 0; ++mit)
	{
		...
		{
			// Remove a monetary value from an item
			int64_t remaind = item->getWorth() - money;
			addMoney(player, remaind, flags);
			money = 0;
		}
		else
			money -= mit->first;
...
Item* Game::transformItem(Item* item, uint16_t newId, int32_t newCount /*= -1*/)
{
	...
	if(curType.alwaysOnTop != newType.alwaysOnTop)
	{
		...
		// std::cout << "**TRANFOR ITEM SYSTEM**_*** IDITEM->> " << item->getID() << " newID ->> " << newId << std::endl;
		delete newItem;
		return NULL;
	}
...
bool Game::playerRequestChannels(uint32_t playerId)
{
	...
	player->sendChannelsDialog(false);//TV Viktor
	return true;
}
...
bool Game::playerOpenChannel(uint32_t playerId, uint16_t channelId)
{
    ...
    if(channelId > 4000 && channelId <= 4100)
    {
	    bool deny = false;
	    CreatureEventList joinEvents = player->getCreatureEvents(CREATURE_EVENT_OPEN_CHANNEL);
	    for(CreatureEventList::iterator it = joinEvents.begin(); it != joinEvents.end(); ++it)
		{
		    if(!(*it)->executeOpenChannel(player, channelId))
                deny = true;
        }
		return deny;
    }
...
bool Game::playerOpenPrivateChannel(uint32_t playerId, std::string& receiver)
{
    ...
    //Player* rec = getPlayerByName(receiver);
	if(IOLoginData::getInstance()->playerExists(receiver))
	{
		//if (rec && rec->getGroupId() < 6)
	...
	const Position& toPos, int16_t toStackpos, uint16_t toSpriteId, bool isHotkey)
{
	...
	if(item->isLoadedFromMap() && !item->isDepot() && !item->isContainer())
	{
		player->sendCancelMessage(RET_CANNOTUSETHISOBJECT);
		return false;
	}
		...
		if(houseTileNo)
		{
		    ...
		    if(houseI && value.type() == typeid(std::string))
		    {
				...
				if(valueB.type() == typeid(int32_t))
				{
					...
					if(poHo)
					{
						//Creature* poHo = getCreatureByID((uint32_t)pokeH);
						//if(!poHo)
						//{
						//	player->sendTextMessage(MSG_INFO_DESCR, "porra npc.");
						//	return false;
						//}
					...
					//player->sendTextMessage(MSG_INFO_DESCR, "porra viado.");
					//		return false;
				}
				...
				//(item)->setAttribute("pokeH", (int32_t)(Creature*)npc->getID());

				internalTeleport((Creature*)npc, pos, true);

				//SpectatorVec list;
       			//getSpectators(list, (Creature*)npc->getPosition());
       			//Player* playerS = NULL;
        		//(Creature*)npc->Nick = poke;
		        ...
		        sprintf(buffer, "%d", atoi(strValue.c_str())+1);
				player->setStorage(87612, buffer);
				//player->sendTextMessage(MSG_INFO_DESCR, (std::string)cc->getID());
				cc->setNick(poke);
         		/*for(SpectatorVec::const_iterator it = list.begin(); it != list.end(); ++it)
        		{
           			if(playerS = (*it)->getPlayer())
               		{
                   		playerS->sendCreatureNick((Creature*)npc);
               		}
        		}*/
	...
	uint32_t creatureId, uint16_t spriteId, bool isHotkey)
{
	...
	if(!Position::areInRange<15,11,0>(creature->getPosition(), player->getPosition()))
		return false;
	...
	uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	if(pos.x == 65535 && pos.y == 7 || pos.x == 65535 && pos.y == 7)
    {
	    player->sendTextMessage(MSG_INFO_DESCR, "sorry, not possible.");
 	 	return false;
    }
...
{
	std::stringstream ss;
if(ret == RET_SIXPOKEMONSREACHED)
	{
		ss << "You have six pokemon";

	}
	else if(ret == RET_NOTENOUGHCAPACITY)
	{
	...
	bool ignoreCap/* = false*/, bool inBackpacks/* = false*/)
{
    ...
    if(amount < 1)
	{
		player->sendTextMessage(MSG_STATUS_SMALL, "Atualize o client");
		return false;
	}
...
bool Game::playerLookInShop(uint32_t playerId, uint16_t spriteId, uint8_t count)
{
	...
	std::stringstream ss;
	ss << Item::getDescription(it, 1, NULL, subType);
	if(player->hasCustomFlag(PlayerCustomFlag_CanSeeItemDetails))
...
bool Game::playerLookAt(uint32_t playerId, const Position& pos1, uint16_t spriteId, int16_t stackpos)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;
	...
	Position pos = pos1;
	if(spriteId == 470 && pos1.z < 7)
	{

		//pos.x -= (8-pos.z);
		//pos.y -= (8-pos.z);

		pos.x -= (7-pos.z);//(8-pos.z)+();
		pos.y -= (7-pos.z);//(8-pos.z);
		pos.z = 7;
	}

	//const Position& pos = pos2;//naonecessa
    ...
    ss << thing->getDescription(lookDistance);
	if(thing->getCreature()->isSummon() || thing->getCreature()->getMonster() || thing->getCreature()->getNpc())
	ss << "Você está vendo " << creature->getName();

	if(thing->getCreature()->isSummon())
    ss << thing->getCreature()->getNickss().c_str() << ". Ele pertence à " << creature->getMaster()->getName();
...
bool Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId)
{
	...
	if(attackCreature->getNpc())
	{
		...
		attackCreature->getNpc()->onCreatureSay(creature, type, text);
		attackCreature->getNpc()->onCreatureSay(creature, type, "trade");
		player->setAttackedCreature(NULL);
		player->sendCancelTarget();
		return false;
	}
    ...
    if (attackCreature->getMonster())
        attackCreature->getMonster()->addToBlackList(player->getCreature());
...
bool Game::playerRequestAddVip(uint32_t playerId, const std::string& vipName)
{
	...
	Player* rec = getPlayerByName(name);
	...
	if(specialVip && !player->hasFlag(PlayerFlag_SpecialVIP))
	{
	 ...
	 if (rec && rec->getGroupId() == 6)
	 {
     player->sendTextMessage(MSG_STATUS_SMALL, "You nao pode add um adm.");
	 return false;
	 }
...
bool Game::pokeChangeOutfit(uint32_t playerId, Outfit_t outfit)
{
	Player* player = getPlayerByID(playerId);
	Creature* creature = getCreatureByID(player->getIDPad());

	if(!creature || creature->isRemoved())
		return false;

	internalCreatureChangePokeOutfit(creature, outfit);
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, SpeakClasses type, const std::string& receiver, const std::string& text)
{
	...
	if(player->isTvWatching())
	{
		if (type == SPEAK_SAY) {
			...
			player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo1");
			return false;
		}
		switch(type)
		{
			case SPEAK_WHISPER:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo2");
			    return false;
			}
				//return playerWhisper(player, text);
			case SPEAK_YELL:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo3");
			    return false;
			}
				//return playerYell(player, text);
			case SPEAK_PRIVATE:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo4");
			    return false;
			}
			case SPEAK_PRIVATE_RED:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo5");
			    return false;
			}
			case SPEAK_RVR_ANSWER:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo6");
			    return false;
			}
				//return playerSpeakTo(player, type, receiver, text);
			case SPEAK_CHANNEL_O:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo7");
			    return false;
			}
			case SPEAK_CHANNEL_Y: //canais privado
			{
				if(channelId >= 100 && channelId <= 10000)
			    {
				...
				player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo8");
			    return false;
			}
			case SPEAK_CHANNEL_RN:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo9");
			    return false;
			}
			case SPEAK_CHANNEL_RA:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo10");
			    return false;
			}
			case SPEAK_CHANNEL_W:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo11");
			    return false;
			}
			/*{
				if(playerTalkToChannel(player, type, text, channelId))
			...
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo12");
			    return false;
			}
			case SPEAK_BROADCAST:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo13");
			    return false;
			}
				//return playerBroadcastMessage(player, SPEAK_BROADCAST, text);
			case SPEAK_RVR_CHANNEL:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo14");
			    return false;
			}
				//return playerReportRuleViolation(player, text);
			case SPEAK_RVR_CONTINUE:
			{
			    player->sendTextMessage(MSG_STATUS_SMALL, "Esta ação não e possivel você esta assistindo15");
			    return false;
			}
...
void Game::internalCreatureChangePokeOutfit(Creature* creature, const Outfit_t& outfit, bool forced/* = false*/)
{
	if(!forced)
	{
		bool deny = false;
	...
	{
		if((tmpPlayer = (*it)->getPlayer()))
			tmpPlayer->sendCreatureChangePokeOutfit(creature, outfit);
	}
...
//			damage = std::min(target->getHealth(), damage);
			if(damage > 0)
			{
                if (target && attacker && target->getPlayer() && attacker->getCreature() && target->getSummonCount() > 0)
                    return false;

                //passive system
				bool deny = false;
                if (target && attacker && target->getMonster())
                    target->getMonster()->addToBlackList(attacker->getCreature());
...
//----Duel System------------
bool Game::playerInviteToDuel(uint32_t playerId, uint32_t invitedId)
{
	if (playerId == invitedId){
		return false;
	}
	...
	Player* invitedPlayer = getPlayerByID(invitedId);
	if(!invitedPlayer || invitedPlayer->isRemoved() || invitedPlayer->isDuelInviting(player))
		return false;

	if(invitedPlayer->getDuel())
	{
		char buffer[90];
		sprintf(buffer, "%s is already in a duel.", invitedPlayer->getName().c_str());
		player->sendTextMessage(MSG_INFO_DESCR, buffer);
		return false;
	}
	...
	if(!duel)
		duel = new Duel(player);
	else if(duel->getLeader() != player)
		return false;

	return duel->invitePlayer(invitedPlayer);
}

bool Game::playerOkDuel(Player* player, Player* invitedPlayer)
{
	Duel* duel = player->getDuel();
	return duel->okDuel(invitedPlayer);
}

bool Game::playerCancelDuel(Player* player)
{
    if(!player || player->isRemoved() || !player->getDuel())
		return false;

	return player->getDuel()->leave(player);
}

bool Game::playerJoinDuel(uint32_t playerId, uint32_t leaderId)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;
	...
	if(!leader || leader->isRemoved() || !leader->isDuelInviting(player))
		return false;

	if(!player->getDuel())
		return leader->getDuel()->join(player);

	player->sendTextMessage(MSG_INFO_DESCR, "You are already in a duel.");
	return false;
}

bool Game::playerRevokeDuelInvitation(uint32_t playerId, uint32_t invitedId)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved() || !player->getDuel() || player->getDuel()->getLeader() != player)
		return false;

	Player* invitedPlayer = getPlayerByID(invitedId);
	if(!invitedPlayer || invitedPlayer->isRemoved() || !player->isDuelInviting(invitedPlayer))
		return false;

	player->getDuel()->revokeInvitation(invitedPlayer);
	return true;
}

bool Game::playerLeaveDuel(uint32_t playerId)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;

	if(!player->getDuel() || player->hasCondition(CONDITION_INFIGHT))
		return false;
...
bool Game::doPlayerAddItemStacking(Player* player, Container* container, uint16_t itemid, uint8_t quant)
{
	for(ContainerIterator it = container->begin(), end = container->end(); it != end; ++it)
    {
        ...
        {
			boost::any value = item->getAttribute("unico");
            if(item->isStackable() && value.empty() && item->getItemCount() < 1000)
            {
...
bool Game::requestSellMarket(uint32_t playerId, const Position& pos, int16_t stackpos, uint16_t spriteId)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;

	Item* marketItem = dynamic_cast<Item*>(internalGetThing(player, pos, stackpos, spriteId, STACKPOS_USE));

	if(!marketItem || marketItem->getClientID() != spriteId || !marketItem->isPickupable() || marketItem->isNotMoveable() || (marketItem->isLoadedFromMap() && (marketItem->getUniqueId() != 0 || (marketItem->getActionId() != 0 && marketItem->getContainer()))))
	{
		player->sendExtendedOpcode(155, "Desculpe, mas voce nao pode vender este item.");
		return false;
	}

	if(player->getPosition().z > marketItem->getPosition().z)
	{
		player->sendExtendedOpcode(155, "Desculpe, mas voce nao pode vender este item.");
		return false;
	}
	...
	if(!Position::areInRange<1,1,0>(marketItem->getPosition(), player->getPosition()))
	{
		player->sendExtendedOpcode(155, "Desculpe, mas voce nao pode vender este item.");
		return false;
	}

	Container* marketContainer = marketItem->getContainer();
	if(marketContainer && marketContainer->getItemHoldingCount() + 1 > 1)
	{
		player->sendExtendedOpcode(155, "Desculpe, mas voce nao pode vender bag com itens dentro.");
		return false;
	}

	if(marketItem->getWorth() > 0)
	{
		player->sendExtendedOpcode(155, "Desculpe, mas voce nao pode vender este item.");
		return false;
	}

	boost::any value = marketItem->getAttribute("unico");
    if(value.type() == typeid(int32_t) && boost::any_cast<int32_t>(value) == 1)
    {
        player->sendCancel("Voce nao pode vender item exclusivo.");
        return false;
    }

    player->marketItem = marketItem;

	player->sendMarketItemRequest(spriteId, marketItem->getItemCount(), marketItem->getName());//manda info para o select do market

	return true;
}

bool Game::acceptSellMarket(uint32_t playerId, uint16_t count, uint64_t price, bool type_money, bool type_unity)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;

	Item* marketItem = player->marketItem;
	if(!marketItem)
	{
		return false;
	}

	if(IOMarket::getMarket()->sellMarket(player, marketItem, count, price, type_money, type_unity))
	{
		player->sendExtendedOpcode(155, "Venda concluida com sucesso.");
	}

	player->marketItem = NULL;

	return true;
}

bool Game::playerCloseMarket(uint32_t playerId)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;

    if(player->getMarketItem())
	{
		player->marketItem = NULL;
	}

    player->sendTextMessage(MSG_STATUS_SMALL, "Market cancelled.");
	return true;
}
