...
PokeBarList Player::pokeBarList;
	...
	Creature(), transferContainer(ITEM_LOCKER), name(_name), nameDescription(_name), client(p)
{
	...
	TvCam = false; //TV Viktor
	TvWatch = false; //TV Viktor
	TvCamIdWatch = NULL;
	nextTvSpectator = 0;
	tv.clear();
...
/*int8_t Player::pokemonCountIn(const Item* item) const{
	...
	Item* itemp = (Item*)item;
    if(value.type() == typeid(std::string))
	{
        count += 1;
		//itemp->setAttribute("ballorder", count);
	}
        ...
        for(ContainerIterator it = container->begin(), end = container->end(); it != end; ++it)
      	{
            ...
            if(value.type() == typeid(std::string)){
                    count += 1;
					//(*it)->setAttribute("ballorder", count);
                }
            if((*it)->getContainer()){  
                count += pokemonCountIn((*it));
            }
        }
    }
    return count;
...
{
	std::stringstream s;
	Town* townss = Towns::getInstance()->getTown(town);
	if(lookDistance == -1)
	{
		s << "Voc� se v�.";
		if(hasFlag(PlayerFlag_ShowGroupNameInsteadOfVocation))
			s << " Voc� � " << (sex % 2 ? "um " : "uma ") << group->getName();
		else if(vocation_id != 0)
			s << " Voc� � " << (sex % 2 ? "um " : "uma ") << vocation->getDescription() << " de " << townss->getName().c_str();
		else
			s << " You have no vocation";
	}
	else
	{
		s << "Voc� est� vendo " << nameDescription;
		if(!hasCustomFlag(PlayerCustomFlag_HideLevel))
			s << " (Level " << level << ")";

		s << ". " << (sex % 2 ? "Ele" : "Ela");
		if(hasFlag(PlayerFlag_ShowGroupNameInsteadOfVocation))
			s << " � " << (sex % 2 ? "um " : "uma ") << group->getName();
		else if(vocation_id != 0)
			s << " � " << (sex % 2 ? "um " : "uma ") << vocation->getDescription();
		else
			s << " has no vocation";
	...
	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i) 
	{
		if(Item* item = getInventoryItem((slots_t)i))
		{
			if(!item->getContainer())
		        inventoryWeight += pokemonCountIn(item);
			else
				inventoryWeight += pokemonCountIn(item);
		}
...
void Player::doUpdatePokemonsBar()
{
	...
	{
		PokeBarList::iterator it = pokeBarList.find(i);
		if(it != pokeBarList.end())
			pokeBarList.erase(i);
	}
		...
		{ 
            ...
            if(value.type() == typeid(std::string))
            {
				...
				if(tapot.type() == typeid(int32_t))
				{
					...
					boost::any levelpo = (item)->getAttribute("level");
					...
					if(hpPo.type() == typeid(float))
					    hpPok = (int32_t)(boost::any_cast<float>(hpPo)*100);
					else if(hpPo.type() == typeid(int32_t))
						hpPok = (int32_t)(boost::any_cast<int32_t>(hpPo)*100);
					
					if(levelpo.type() == typeid(int32_t))
						level = boost::any_cast<int32_t>(levelpo);
                    ...
                    if(border.type() == typeid(int32_t))
                    {
						s << "," << foto->clientId << "|" << boost::any_cast<std::string>(value) << "|" << boost::any_cast<int32_t>(border) << "|" << hpPok << "|" << boost << "|" << level << "|" << sexo;
					
						std::string tx = s.str();
					    pokeBarList[boost::any_cast<int32_t>(border)] = tx;  
					    s.str("");
                	    bo += 1;
					}
				}else
				{
                    ...
                    if(border.type() == typeid(int32_t))
                    {
						s << "," << foto->clientId << "|" << "" << "|" << boost::any_cast<int32_t>(border) << "" << "|" << "" << "|" << "" << "|" << "";
						std::string tx = s.str();
					    pokeBarList[boost::any_cast<int32_t>(border)] = tx;
					    s.str("");
                	    bo += 1; 
                ...
                for(ContainerIterator it = container->begin(), end = container->end(); it != end; ++it)
      	        {
                    ...
                    if(value.type() == typeid(std::string))
                    {
				        ...
				        if(tapot.type() == typeid(int32_t))
				        {
							...
							boost::any levelpo = (*it)->getAttribute("level");
							...
							boost::any border = (*it)->getAttribute("ballorder");
                            if(border.type() == typeid(int32_t))
                            {
								s << "," << foto->clientId << "|" << boost::any_cast<std::string>(value) << "|" << boost::any_cast<int32_t>(border) << "|" << hpPok << "|" << boost << "|" << level << "|" << sexo;
	...
	for (uint8_t i = 1; i <= 6; ++i)
	{
		s << pokeBarList[i];//it->second;
	}
    sendCancel(s.str());
}

void Player::updateLosesBall(uint8_t values)
{
 	...
 	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
		...
		{ 
            ...
            if(value.type() == typeid(std::string))
			{
                (item)->setAttribute("loses", values);
            }   
            if(container = item->getContainer())
			{
                for(ContainerIterator it = container->begin(), end = container->end(); it != end; ++it)
...
bool Player::canWalkthrough(const Creature* creature) const
{
	...
	std::string flySto;
	if(creature->getPlayer() && creature->getStorage(17000, flySto) && atoi(flySto.c_str()) >= 1)
		return false;
    ...
    // if (creature->getTile()->ground && creature->getTile()->ground->getID() == 11644) {
    //     return true;
    // }
...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_DONTSHOWMESSAGE:
			break;
        case RET_SIXPOKEMONSREACHED:
            sendCancel("You can't carry more pokemons.");
            break;
...
{
	Creature::onWalk(dir);
	Creature::setStatValue("ordered", 0);
	Creature::setOrderHabil("");
	Position odjff = Creature::getOrderPos();
	Creature::setOrderPos(odjff - odjff);
	...
	uint32_t flags) const
{
	...
	if(destItem)
	{
		if(destItem->isStackable() && item->getID() == destItem->getID())
			maxQueryCount = 1000 - destItem->getItemCount();//65kitem
		else
			maxQueryCount = 0;
	}
	else
    ...
    uint32_t& flags)
{
    if (index == 0 /*drop to capacity window*/ || index == INDEX_WHEREEVER)
    {
		*destItem = NULL;
        ...
        // find an appropiate slot -- desativado mixlort bug do /i
		for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
		{
...
void Player::onAttackedCreatureDrain(Creature* target, int32_t points)
{
	...
	if (target->getMonster()){
        ...
        target->getMonster()->addToBlackList(this->getCreature());
    }
	// sprintf(buffer, "Your pokemon dealt %d damage to %s.", points, target->getNameDescription().c_str()); // mixlort
	char bufferA[100];
	sprintf(bufferA, "-%d", points);
	g_game.addAnimatedText(target->getPosition(), 25, bufferA); 
...
bool Player::canWearPokeOutfit(uint32_t outfitId, Creature* creature)
{
	OutfitMap::iterator it = outfits.find(outfitId);
	std::string value;
	if (creature->getStorage(it->second.storageId, value) && value == it->second.storageValue)
		return true;
...
void Player::eraseGuildInvited(uint32_t guildId)
{
	/*for(InvitedToGuildsList::iterator it = invitedToGuildsList.begin(); it != invitedToGuildsList.end(); ++it)
	{
		if((*it) == guildId)
		{
			invitedToGuildsList.erase(it);
		}
	}*/
	...
	InvitedToGuildsList::iterator it = std::find(invitedToGuildsList.begin(), invitedToGuildsList.end(), guildId);
	if(it != invitedToGuildsList.end())
		return;
...
PartyShields_t Player::getDuelShield(const Creature* creature) const
{
	const Player* player = creature->getPlayer();
	if(!player)
		return Creature::getPartyShield(creature);
	...
	{
		if(duel->getLeader() == player)
		{

			return SHIELD_YELLOWDUEL;
		}

		if(duel->isPlayerMember(player))
		{

			return SHIELD_BLUEDUEL;
		}

		if(isDuelInviting(player))
			return SHIELD_WHITEBLUEDUEL;
	}

	if(player->isDuelInviting(this))
		return SHIELD_WHITEYELLOWDUEL;
...
bool Player::isDuelInviting(const Player* player) const
{
	if(!player || !getDuel() || getDuel()->getLeader() != this)
		return false;

	return getDuel()->isPlayerInvited(player);
}
...
//-----Duel System----------
void Player::sendPlayerDuelIcons(Player* player)
{
	sendCreatureDuelShield(player);
//	sendCreatureSkull(player);
}

bool Player::addDuelInvitation(Duel* duel)
{
	if(!duel)
		return false;

	DuelList::iterator it = std::find(inviteDuelList.begin(), inviteDuelList.end(), duel);
	if(it != inviteDuelList.end())
		return false;

	inviteDuelList.push_back(duel);
	return true;
}

bool Player::removeDuelInvitation(Duel* duel)
{
	if(!duel)
		return false;
	...
	if(it != inviteDuelList.end())
	{
		inviteDuelList.erase(it);
		return true;
	}
	return false;
}

void Player::clearDuelInvitations()
{
	if(inviteDuelList.empty())
		return;

	DuelList list;
	for(DuelList::iterator it = inviteDuelList.begin(); it != inviteDuelList.end(); ++it)
		list.push_back(*it);

	inviteDuelList.clear();
	for(DuelList::iterator it = list.begin(); it != list.end(); ++it)
...
void Player::setPlayerCaught(std::string _poke, uint8_t typec, bool count)
{
    if(_poke.empty())
        return;
                 
    MonsterType* mType = g_monsters.getMonsterType(_poke);
    if(!mType)
        return;
         
    PlayerCatch::iterator it = playerCatch.find(_poke);
    if(it != playerCatch.end())
    {
...
void Player::setPlayerCatchCount(std::string _poke, uint8_t typec, uint16_t count) 
{
    if(_poke.empty())
        return;
    ...
    if(it == playerCatch.end())
    {
        playerCatch[_poke] = catchPlayer_t(_poke, false, false, false, 0, 0, 0, 0, 0, 0, 0, 0); 
        it = playerCatch.find(_poke);		
	}
    ...
    if(it != playerCatch.end())  
    {
        switch(typec)
        {
            case POKE:
            {
                it->second.pokeball = count;
                break;
            }
            case GREAT:
            {
                it->second.greatball = count;
                break;
            }
            case SUPER:
            {
                it->second.superball = count;
                break;
            }
            case ULTRA:
            {
                it->second.ultraball = count;
                break;
            }
            case SAFFARI:
            {
                it->second.saffariball = count;
                break;
            }
            case MASTER:
            {
                it->second.masterball = count;
                break;
            }
			case PREMIER:
            {
                it->second.premierball = count;
                break;
            }
			case VBALL:
            {
                it->second.vball = count;
                break;
            }
            default:
			    break;
...
uint16_t Player::getPlayerCatchCount(std::string _poke, uint8_t typec)
{
    if(_poke.empty())
        return 0;
    ...
    if(it != playerCatch.end())
    {
        ...
        switch(typec)
        {
            case POKE:
            {
                  count = it->second.pokeball;
                  break;
            }
            case GREAT:
            {
                  count = it->second.greatball;
                  break;
            }
            case SUPER:
            {
                  count = it->second.superball;
                  break;
            }
            case ULTRA:
            {
                   count = it->second.ultraball;
                   break;
            }
            case SAFFARI:
            {
                  count = it->second.saffariball;
                  break;
             }
            case MASTER:
            {
                  count = it->second.masterball;
                  break;
            }
            case PREMIER:
            {
                  count = it->second.premierball;
                  break;
            }
			case VBALL:
            {
                count = it->second.vball;
                break;
            }
            default:
			    break;
...
void Player::setPlayerCatch(std::string _poke, bool _dex, bool _use, bool _catch)
{
    if(_poke.empty())
        return;
    ...
    { 
          it->second.dex = _dex;
          it->second.use = _use;
          it->second.catchP = _catch;
          return; 
    }
    
    playerCatch[_poke] = catchPlayer_t(_poke, _dex, _use, _catch, 0, 0, 0, 0, 0, 0, 0, 0);
}
        
bool Player::getPlayerCatch(std::string _poke, std::string _key)
{
    if(_key == "dex")
    {
        PlayerCatch::iterator it = playerCatch.find(_poke);
...
//TV Viktor
void Player::setTvCam(bool _TvCam)
{
	TvCam = _TvCam;
}
bool Player::isTvCam()
...
void Player::setTvWatching(bool _TvWatch)
{
	TvWatch = _TvWatch;
}
bool Player::isTvWatching()
{
	return TvWatch;
}
		
void Player::setWatchTvCamId(Player* _TvCamIdWatch)
{
	TvCamIdWatch = _TvCamIdWatch;
}

Player* Player::getWatchTvCamId()
{
	return TvCamIdWatch;
}

	
void Player::addWatchInTVCam(ProtocolGame* pg)
{ //adiciona o Procotol do Watch na lista do Camera
			//tv.push_back(pg);
			//Tvlist[nextTvSpectator] = pg;
			tv.push_back(pg->getPlayer());
	//tv[nextTvSpectator] = pg;
	//nextTvSpectator++;
}
		
void Player::sendWatchInTVCam(Player* player_Cam) {
	//if(client) 
	//	client->sendWatchInTVCam(player_Cam); 
    player_Cam->tv.push_back(this);
	this->setWatchTvCamId(player_Cam);
	this->setTvWatching(true);
}
		
void Player::sendUpdateTvWatch(const Position& pos, const Player* player_Cam) const //vai para a visao do camera
{
	if(client) 
		client->sendUpdateTvWatch(pos, player_Cam); 
}
...
{
	if(this->isTvWatching()) //TV Viktor
		return; 
		   
	if(client) 
		client->sendAddTileItem(tile, pos, tile->getClientIndexOfThing(this, item), item);
			
	Tvlist::const_iterator it = tv.begin();  
	for(uint32_t i = 1; it != tv.end(); ++it, ++i)
    { 
		Player* playerWatching = (*it);
		if(playerWatching)
			playerWatching->checkClient()->sendAddTileItemTV(tile, pos, tile->getClientIndexOfThing(this, item), item, this);
	}
}
void Player::sendUpdateTileItem(const Tile* tile, const Position& pos, const Item* oldItem, const Item* newItem)
{
    ...
    { 
		Player* playerWatching = (*it);
		if(playerWatching)
			playerWatching->checkClient()->sendUpdateTileItemTV(tile, pos, tile->getClientIndexOfThing(this, oldItem), newItem, this);
	} 
}
void Player::sendRemoveTileItem(const Tile* tile, const Position& pos, uint32_t stackpos, const Item* item)
{
    ...
    { 
		Player* playerWatching = (*it);
		if(playerWatching)
			playerWatching->checkClient()->sendRemoveTileItemTV(tile, pos, stackpos, this);
	}
}
void Player::sendUpdateTile(const Tile* tile, const Position& pos)
{
    ...
    { 
		Player* playerWatching = (*it);
		if(playerWatching)
			playerWatching->checkClient()->sendUpdateTileTV(tile, pos, this);
	} 
}
...
void Player::sendCreatureAppear(const Creature* creature)
{
    ...
    { 
		Player* playerWatching = (*it);
		if(playerWatching)
			playerWatching->checkClient()->sendAddCreatureTV(creature, creature->getPosition(), creature->getTile()->getClientIndexOfThing(this, creature), this);
	}
}
void Player::sendCreatureDisappear(const Creature* creature, uint32_t stackpos)
{
    ...
    { 
		Player* playerWatching = (*it);
		if(playerWatching)
			playerWatching->checkClient()->sendRemoveCreatureTV(creature, creature->getPosition(), stackpos, this);
	}
}
void Player::sendCreatureMove(const Creature* creature, const Tile* newTile, const Position& newPos, const Tile* oldTile, const Position& oldPos, uint32_t oldStackpos, bool teleport)
{
	if(this->isTvWatching()) //TV Viktor
		return; 
	...
	{	
		...
		{
			Player* playerWatching = (*it);
			if(creature->getPlayer() && playerWatching && creature->getPlayer() == this)
			{ 
				playerWatching->checkClient()->sendMoveCreatureTV(this, newTile, newPos, newTile->getClientIndexOfThing(this, creature), oldTile, oldPos, oldStackpos, teleport, true, this);
				//playerWatching->sendCancel("[TV System]\n\nWrong password to protected channel.");
			}else if(playerWatching)  
			{ 
				playerWatching->checkClient()->sendMoveCreatureTV(creature, newTile, newPos, newTile->getClientIndexOfThing(this, creature), oldTile, oldPos, oldStackpos, teleport, false, this);
				//playerWatching->sendCancel("Monstrosn Andando."); 
			}
		}  
	}
}
...
void Player::sendCreatureTurn(const Creature* creature)
{
	...
	for(uint32_t i = 1; it != tv.end(); ++it, ++i)
	{
		Player* playerWatching = (*it);
		if(playerWatching)  
			playerWatching->checkClient()->sendCreatureTurnTV(creature, creature->getTile()->getClientIndexOfThing(this, creature), this); 
	} 
}
...
void Player::sendCreatureChangeOutfit(const Creature* creature, const Outfit_t& outfit)
{
	...
	for(uint32_t i = 1; it != tv.end(); ++it, ++i)
	{
		Player* playerWatching = (*it);
		if(playerWatching)
			playerWatching->checkClient()->sendCreatureOutfitTV(creature, outfit, this);
	}
}

void Player::sendCreatureHealth(const Creature* creature) const//TV
{
	Player* merda = (Player*)this;
	if(merda->isTvWatching()) //TV Viktor
		return; 
	...
	for(uint32_t i = 1; it != tv.end(); ++it, ++i)
	{
		Player* playerWatching = (*it);
		if(playerWatching)
			playerWatching->checkClient()->sendCreatureHealthTV(creature, (Player*)this);
	}
}
void Player::sendDistanceShoot(const Position& from, const Position& to, uint8_t type) const//TV
{
	Player* merda = (Player*)this;
	if(merda->isTvWatching()) //TV Viktor
		return; 
	...
	for(uint32_t i = 1; it != tv.end(); ++it, ++i) 
	{
		Player* playerWatching = (*it);
		if(playerWatching)
			playerWatching->checkClient()->sendDistanceShootTV(from, to, type, (Player*)this);
	}
}
...
void Player::sendMagicEffect(const Position& pos, uint16_t type) const
{
	...
	for(uint32_t i = 1; it != tv.end(); ++it, ++i)
	{
		...
		if(playerWatching)   
			playerWatching->checkClient()->sendMagicEffectTV(pos, type, (Player*)this); 
	}
}  

void Player::setPvpArenaBans(std::string poke)  
{
	//ArenaBansList::iterator it = pvpArenaBans.find(poke);
	//if(it != pvpArenaBans.end())
		pvpArenaBans[poke] = this;
}

void Player::setPvpArenaPicks(std::string poke) 
{
	//ArenaPicksList::iterator it = pvpArenaPicks.find(poke);
	//if(it != pvpArenaPicks.end())
		pvpArenaPicks[poke] = this;
}	

void Player::doErasePvpArenaDepot()
{
    const Container* container;
 	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
        if(Item* item = getInventoryItem((slots_t)i)) 
		{ 
            boost::any value = (item)->getAttribute("pvparena"); 
            if(!value.empty())
			{
				boost::any poke = (item)->getAttribute("poke"); 
				std::cout << "attributesSize >> " << boost::any_cast<std::string>(poke) << std::endl;
				g_game.internalRemoveItem(NULL, item);
				continue;
			}
                ...
                for(ContainerIterator it = container->begin(), end = container->end(); it != end; ++it)
      	        {
					...
					if(newItem)
                    {
						value = newItem->getAttribute("pvparena");
                    	if(!value.empty())
						{
							boost::any poke = newItem->getAttribute("poke");
							//std::string poke = boost::any_cast<std::string>(value);
							
							std::cout << "attributesSize >> " << boost::any_cast<std::string>(poke) << std::endl;
							g_game.internalRemoveItem(NULL, newItem); 
						}
					}
                }
            }
...
// {
// 	if (g_game.getMoney(this) < price * countSelectedScrollBar) {
...
// 				g_game.addMoney(player, price * countSelectedScrollBar);
...
// 	uint64_t page_numeration = 0; // determinar a pagina que o player tï¿½
