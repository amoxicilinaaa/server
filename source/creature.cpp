...
bool Creature::canSee(const Position& myPos, const Position& pos, uint32_t viewRangeX, uint32_t viewRangeY)
{
	...
	return (((uint32_t)pos.x >= myPos.x - Map::maxViewportX + offsetz) && ((uint32_t)pos.x <= myPos.x + Map::maxViewportX + offsetz) &&
		((uint32_t)pos.y >= myPos.y - Map::maxViewportY + offsetz) && ((uint32_t)pos.y <= myPos.y + Map::maxViewportY + offsetz));
}
	...
	if(creature->isPlayerSummon())
{
    if(!creature->getMaster()->hasCondition(CONDITION_INFIGHT))
    {
        ...
        if(creature->getName() == "Gastly")
            fpp.clearSight = false;
		if(creature->getName() == "Darkrai")
            fpp.clearSight = false;
		if(creature->getName() == "Darkrai Nightmare")
            fpp.clearSight = false;
    }
}
	fpp.clearSight = true;
