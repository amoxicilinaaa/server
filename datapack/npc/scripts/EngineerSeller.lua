...
function creatureSayCallback(cid, type, msg)
	...
	if isInArray({"Help", "Engineer"}, doCorrectString(msg)) then
		if getPlayerProfessionId(cid) == 2 then
			itemsName = {}
			for i = 1, #SellEnginner do
				if getPlayerLevel(cid) >= SellAdventurer[i].level then
					table.insert(itemsName, i, "{"..doCorrectString(getItemNameById(SellEnginner[i].id)).."}")
				end
			end
			selfSay("Eu posso te ajudar a fazer ".. doConcatTable(itemsName, ", ", " ou ") ..".", cid)
			talkState[talkUser] = 1
		else
			selfSay("Você não é engenheiro!", cid)
		end
	elseif talkState[talkUser] == 1 then
		...
		if isInArray(itemsName, mesg) then
			...
			id = 0
			for i = 1, #SellEnginner do
				if itemid == SellEnginner[i].id then
					id = i
				end
			end	
			if SellEnginner[id] then
				tab = SellEnginner[id]
