...
function creatureSayCallback(cid, type, msg)
	...
	if isInArray({"Help", "Teacher"}, doCorrectString(msg)) then
		if getPlayerProfessionId(cid) == 4 then
			itemsName = {}
			for i = 1, #SellTeacher do
				table.insert(itemsName, i, "{"..doCorrectString(getItemNameById(SellTeacher[i].id)).."}")
			end
			selfSay("Eu posso te ajudar a fazer ".. doConcatTable(itemsName, ", ", " ou ") ..".", cid)
			talkState[talkUser] = 1
		else
			selfSay("Você não é professor!", cid)
		end
	elseif talkState[talkUser] == 1 then
		...
		if isInArray(itemsName, mesg) then
			...
			id = 0
			for i = 1, #SellTeacher do
				if itemid == SellTeacher[i].id then
					id = i
				end
			end	
			if SellTeacher[id] then
				tab = SellTeacher[id]
