    ...
    if(not npcHandler:isFocused(cid)) then
        ...
        elseif talkState[talkUser] == 4 then
                if msgcontains(msg, 'proceed') then
                        ...
                        if getPlayerItemCount(cid,2124) >= 1 and  getPlayerItemCount(pid,2124) >= 1 then --- item que precisa pra casar
                        
                        selfSay('voce precisa do anel de casamento',cid)
                
end
                                        setPlayerStorageValue(cid,3066,2)
                                        setPlayerStorageValue(pid,3066,2)
                                        setPlayerStorageValue(cid,150420,1) --- aki ae sto que ta na roupa no outfit.xml (quests="150420") bote o seu
                                        setPlayerStorageValue(pid,150420,1)--- aki ae sto que ta na roupa no outfit.xml (quests="150420") bote o seu
                                        doPlayerRemoveItem(cid,2124,1) ---- item q precisa pra casar
                                        doPlayerRemoveItem(pid,2124,1)---- item q precisa pra casar
                                        doCancelMarryStatus(fid)
                                        doCancelMarryStatus(sid)
                                        setPlayerPartner(cid,getPlayerGUID(pid))
                                        setPlayerPartner(pid,getPlayerGUID(cid))
              
                                        
                                        doCreatureSay(cid, text[chance1], TALKTYPE_ORANGE_1)
                                        doCreatureSay(pid, text[chance2], TALKTYPE_ORANGE_1)
                                        doSendMagicEffect(tmf, 147)
                                        doSendMagicEffect(tms, 147)
