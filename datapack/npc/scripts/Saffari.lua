...
following = false
attacking = false
local SafariEnter = {x=1168, y=1457, z=7} -- Posição da Entrada Safari
...
if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 3 then
   selfSay('Ola a entrada no Saffari custara 20HDs vc aceita??')
   focus = cid
   talk_start = os.clock()
elseif (msgcontains(msg, 'yes') ) then
   if getPlayerItemCount(cid,2392) >= 1 or getPlayerItemCount(cid,2393) >= 1 or getPlayerItemCount(cid,2391) >= 1 or getPlayerItemCount(cid,14432) >= 1 then
      selfSay("Voce nao pode entrar no Saffari com nenhuma outra ball exceto a Saffari!")
      focus = 0
      talk_start = 0
