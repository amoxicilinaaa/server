...
if not canSummon then
    ...
    local function growHead()
          doTransformItem(getTileItemById(pos, 2702).uid, 2703) --id do item  arvore quebrada, arvore normal
    end
...
----------------------------------------------------------------------
	...
	--local condition = createConditionObject(CONDITION_RET_CANNOTBEDRESSED)
	
	
if getPlayerStorageValue(cid, 16701) >= 1 then 
doPlayerSendTextMessage(cid, 19, "Nao Pode Dar Order no capture a bandeira.")
return true
end

   
        local premium = false --- Coloque false caso não queira usar
 
          if isPlayer(cid) and not isPremium(cid) and premium == true then
          doPlayerSendCancel(cid, "Apenas jogadores VIPs podem usar o Fly.")
              return true
              end
			  
			  if getTilePzInfo(getCreaturePosition(cid)) then	  

		 doPlayerSendCancel(cid,"Você não pode usar oder em PZ")	  
		...
		doPlayerSendCancel(cid, "##system##ENBUD")
		doChangeSpeed(cid, -getCreatureSpeed(cid))
		doChangeSpeed(cid, 1160)
		setPlayerStorageValue(cid, 54844, 1160)
		doSetCreatureOutfit(cid, {lookType = pokemon[1] + 351}, -1)
		local pball = getPlayerSlotItem(cid, 8)
		local addon = getItemAttribute(pball.uid,"addon") or 0
		if addon > 0 then
			doSetCreatureOutfit(cid, {lookType = addonsaction[addon].fly}, -1)
		end
...
return true
		...
		local addon = getItemAttribute(pball.uid,"addon") or 0
		if addon > 0 then
			doSetCreatureOutfit(cid, {lookType = addonsaction[addon].ride}, -1)
		end
		doItemSetAttribute(getPlayerSlotItem(cid, 8).uid, "hp", getCreatureHealth(getCreatureSummons(cid)[1]) / getCreatureMaxHealth(getCreatureSummons(cid)[1]))
		doRemoveCreature(getCreatureSummons(cid)[1])
