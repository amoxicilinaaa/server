...
function doEvolvePokemon(cid, item2, theevo, stone1, stone2)
	...
	local pct = getCreatureHealth(item2.uid) / getCreatureMaxHealth(item2.uid)
		...
		local off = status.off * 14.5
		local def = status.def * 14.5
...
function setWildPokemonLevel(cid, optionalLevel, optionalStatus, optionalNick, optionalExtraExp)
	...
	if optionalLevel and tonumber(optionalLevel) >= 1 then                --alterado v2.8\/
		...
		local wildLvlMin = pokes[this].wildLvl - 5
		local wildLvlMax = pokes[this].wildLvl + 5
		if wildLvlMin < 1 then
			wildLvlMin = 1
		elseif wildLvlMax > 100 then
			wildLvlMax = 100
		end
		if wildLvlMin >= wildLvlMax then
			wildLvlMin = wildLvlMax - 5
		end
		levelRange = math.random(wildLvlMin, wildLvlMax)
	end                                                                                          
