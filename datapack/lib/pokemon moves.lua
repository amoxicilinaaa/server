...
elseif not isPlayer(cid) then
	if table ~= "" then   --alterado v1.6
	    ...
	    min = getSpecialAttack(cid)   --alterado v1.6
...
elseif spell == "Dazzling Gleam" then
	...
	for a = 1, 100 do
		...
		addEvent(doSendLeafStorm, a * 1, cid, lugar)
	end
	
	elseif spell == "Surf Power" then

   	 doMoveInArea2(cid, 238, dazzling, dano, min, max, spell, ret)
	addEvent(doDanoWithProtect, 150, cid, WATERDAMAGE, getThingPosWithDebug(cid), grassarea, -min, -max, 0)
	...
	elseif spell == "Ominus Wind" then

   	 doMoveInArea2(cid, 41, dazzling, dano, min, max, spell, ret)
	addEvent(doDanoWithProtect, 150, cid, DARKDAMAGE, getThingPosWithDebug(cid), grassarea, -min, -max, 0)
	...
	elseif spell == "Dark Psy" then

   	 doMoveInArea2(cid, 40, dazzling, dano, min, max, spell, ret)
	addEvent(doDanoWithProtect, 150, cid, DARKDAMAGE, getThingPosWithDebug(cid), grassarea, -min, -max, 0)
...
local eff = {42, 2, 42, 42}
local area = {windstorm1, windstorm2, windstorm1, windstorm2}

addEvent(doMoveInArea2, 2*450, cid, 2, windstorm1, FLYINGDAMAGE, min, max, spell)
for i = 0, 3 do
    addEvent(doMoveInArea2, i*450, cid, eff[i+1], area[i+1], FLYINGDAMAGE, min, max, spell)
end
...
setPlayerStorageValue(cid, 3644587, 1)
addEvent(setPlayerStorageValue, 8000, cid, 3644587, -1)
...
doMoveInArea2(cid, 22, selfArea1, NORMALDAMAGE, 0, 0, spell, ret) 
...
local spells = {"Shadow Storm", "Electric Storm", "Magma Storm", "Blizzard", "Leaf Storm", "Hydropump", "Falling Rocks", "Earthquake", "Stomp", "Dazzling Gleam","Ominus Wind", "Leaf Storm", "Selfdestruct", "Blizzard", "Shadow Storm", "Surf", "Inferno", "Fissure", "Last Resort", "Electro Field", "Petal Tornado", "Flame Wheel"}
...
elseif spell == "Wish" then
	...
	elseif spell == "Vitality Impulse" then
	...
	local EventStart = addEvent(doHealAreatru, 5000, alejadao, math.random(100,200), math.random(400,1000))
	...
	addEvent(checkHealtru, 1, getCreatureMaster(cid))
...
local ret = {id = 0, cd = 13, eff = 34, check = 0, spell = spell, cond = "Miss"} 
	...
	local function gas(cid)
           doMoveInArea2(cid, 114, poisong, POISONDAMAGE, -min, -max, spell, ret)
	end
...
elseif spell == "Mega Punch" then
	...
	elseif spell == "Dark Punch" then

	doSendDistanceShoot(getThingPosWithDebug(cid), getThingPosWithDebug(target), 39)
	doDanoInTargetWithDelay(cid, target, FIGHTINGDAMAGE, -min, -max, 219)  --alterado v1.7
...
addEvent(doMoveInArea2, 100, cid, 2, epicenter1, GROUNDDAMAGE, (min / 1500), (max / 1500), spell)
for i = 0, 3 do
    addEvent(doMoveInArea2, i*1, cid, eff[i+1], area[i+1], GROUNDDAMAGE, 0, 0, spell)
end
        
elseif spell == "Ice Crash" then

local eff = {52, 39, 52, 52}
local area = {epicenter1, epicenter1, epicenter2, epicenter2}

addEvent(doMoveInArea2, 100, cid, 2, epicenter1, GROUNDDAMAGE, (min / 10), (max / 10), spell)
for i = 0, 3 do
    addEvent(doMoveInArea2, i*20, cid, eff[i+1], area[i+1], GROUNDDAMAGE, 0, 0, spell)
end
...
local eff = {194, 194, 151, 157, 42, 151, 42}
local area = {dragondance, dragondance, dragondance1, dragondance2, dragondance1, dragondance2, dragondance1}

addEvent(doMoveInArea2, 2*300, cid, 2, dragondance, DRAGONDAMAGE, min, max, spell)
for i = 0, 7 do
    addEvent(doMoveInArea2, i*300, cid, eff[i+1], area[i+1], DRAGONDAMAGE, min, max, spell)
end

elseif spell == "Electrical Burning" then

local eff = {48, 48, 48, 48}
local area = {dragondance2, dragondance1, dragondance, dragondance2, dragondance1, dragondance, dragondance2}

addEvent(doMoveInArea2, 2*450, cid, 2, dragondance, DRAGONDAMAGE, min, max, spell)
for i = 0, 3 do
    addEvent(doMoveInArea2, i*450, cid, eff[i+1], area[i+1], DRAGONDAMAGE, min, max, spell)
end
...
addEvent(doMoveInArea2, 150, cid, 2, dragondance, DRAGONDAMAGE, min, max, spell)
    addEvent(doMoveInArea2, 150, cid, 100, dragondance2, DRAGONDAMAGE, min, max, spell)


elseif spell == "Aero Crash" then

local eff = {42, 42, 42, 42}
local area = {dragondance2, dragondance1, dragondance, dragondance2, dragondance1, dragondance, dragondance2}
...
local eff = {42, 42, 42, 42}
local area = {wind1, wind2, wind3, wind4}

addEvent(doMoveInArea2, 2*450, cid, 2, wind1, DRAGONDAMAGE, min, max, spell)
for i = 0, 3 do
    addEvent(doMoveInArea2, i*450, cid, eff[i+1], area[i+1], DRAGONDAMAGE, min, max, spell)
end
...
elseif spell == "Great Love" then
        ...
        doMoveInArea2(cid, 147, greatlove, NORMALDAMAGE, min, max, spell)
...
local effD = getSubName(cid, target) == "Shiny Metagross" and 26 or 26 -- 13 é o efeito da "pedrinha" no caso é "missile"
local eff = getSubName(cid, target) == "Shiny Metagross" and 91 or 91 -- 91 é o efeito "Meteor" - no caso é "effect"
...
local effD = getSubName(cid, target) == "Shiny Dragonite" and 26 or 26 -- 13 é o efeito da "pedrinha" no caso é "missile"
local eff = getSubName(cid, target) == "Shiny Dragonite" and 5 or 5 -- 5 é o efeito "Meteor" - no caso é "effect"
...
addEvent(doFall, 100, cid)
addEvent(doDanoWithProtect, 100, cid, ROCKDAMAGE, getThingPosWithDebug(cid), waterarea, -min, -max, 0)

elseif spell == "Rock Stock" then

local effD = getSubName(cid, target) == "Shiny Metagross" and 11 or 11 -- 13 é o efeito da "pedrinha" no caso é "missile"
local eff = getSubName(cid, target) == "Shiny Metagross" and 127 or 127 -- 100 é o efeito "Meteor" - no caso é "effect"
...
elseif spell == "Surf Tower" then

local effD = getSubName(cid, target) == "Shiny Metagross" and 28 or 28 -- 13 é o efeito da "pedrinha" no caso é "missile"
local eff = getSubName(cid, target) == "Shiny Metagross" and 245 or 245 -- 100 é o efeito "Meteor" - no caso é "effect"
...
local function doFall(cid)
for rocks = 1, 62 do
    addEvent(fall, rocks*50, cid, master, WATERDAMAGE, effD, eff)
end
end
...
local effD = getSubName(cid, target) == "Shiny Hitmonchan" and 26 or 26 -- 13 é o efeito da "pedrinha" no caso é "missile"
local eff = getSubName(cid, target) == "Shiny Hitmonchan" and 112 or 35 -- 100 é o efeito "Meteor" - no caso é "effect"
...
local effD = 24 -- 13 é o efeito da "pedrinha" no caso é "missile"
local eff = 52 -- 100 é o efeito "Meteor" - no caso é "effect"
...
local function doFall(cid)
for rocks = 1, 30 do
    addEvent(fall, rocks*32, cid, master, ROCKDAMAGE, effD, eff)
end
end
