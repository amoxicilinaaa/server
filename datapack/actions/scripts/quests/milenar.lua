...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if getPlayerStorageValue(cid, config.bossStorage) >= 1 then
		...
		doPlayerSendTextMessage(cid, 25, "Voce Precisa Achar o Dragonite Milenar")
	
	return false
end

end
doPlayerSendTextMessage(cid, 19 ,"Voce precisa de level 200 para completar a milenar")
return false
end