...
storage = 550982398 -- Storage a cada quest que for criada aumente um numero da storage pra qnd vc pegar o baú de outra quest ñ aparecer que vc já fez.
item = 2183 -- Id do item ira ganhar.
nomeitem = "Addon Box" -- Nome do item
quantidade = 1 -- Quantidade ira ganhar.
level = 230 -- Level que precisa pra fazer.

	if getPlayerLevel(cid) >= level and getPlayerStorageValue(cid,storage) == -1 then -- Não mecha.
doPlayerSendTextMessage(cid,25,"Você ganhou um "..nomeitem..".") -- Mensagem que aparecera quando ganhar o item.
