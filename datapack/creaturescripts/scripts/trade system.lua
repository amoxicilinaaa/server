function onTradeRequest(cid, target, item)
    ...
    if isContainer(item.uid) then                         --alterado v1.8 \/
	   local itens = getPokeballsInContainer(item.uid)
	   if #itens >= 1 then                             
	      for i = 1, #itens do
...
function onTradeAccept(cid, target, item, targetItem)
    ...
    if getPlayerMana(cid) + itemPokeball > 7 then  --alterado v1.6
	   cancel = true
	   p1 = cid
    end
    if getPlayerMana(target) + targetItemPokeball > 7 then  --alterado v1.6
	   cancel = true
	   p2 = target
    end
    
	if pbs > 0 and getCreatureMana(target) + pbs > 7 + targetItemPokeball then
		cancel = true	
		p1 = target
	end
